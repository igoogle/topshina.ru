...
<div class="bx_sort_container tabs">
	...
	<span class="wrap_remove_button">
		...
		?>
		<span class="btn btn-default white grey remove_all_compare icon_close" onclick="CatalogCompareObj.MakeAjaxAction('<?=$arTheme["COMPARE_PAGE_URL"]["VALUE"];?>?action=DELETE_FROM_COMPARE_RESULT&ID[]=<?=$arStr?>', 'Y');"><?=GetMessage("CLEAR_ALL_COMPARE")?></span>
	</span>
</div>
<div class="table_compare wrap_sliders tabs-body">
	<?if($arResult["SHOW_FIELDS"]):?>
		<?if(count($arResult["IBLOCKS"]) > 1):?>
			<div class="filter_type_block">
				<span class="active bg_block_colored colored"><span><?=\Bitrix\Main\Localization\Loc::getMessage("YMGRADE_0");?></span></span>
				<?foreach($arResult["IBLOCKS"] as $key => $value):?>
					<span class="bg_block_colored colored" data-iblock="<?=$key;?>"><span><?=$value;?></span></span>
				<?endforeach;?>
			</div>
		<?endif;?>
		<div class="frame top">
				...
				<table class="compare_view top">
					<tr>
						<?foreach($arResult["ITEMS"] as &$arElement){?>
							<td data-iblock="<?=$arElement["IBLOCK_ID"];?>">
								<div class="item_block">
									...
									<div class="image_wrapper_block">
										...
										<?}elseif($arElement["FIELDS"]["DETAIL_PICTURE"]){
											...
											<?endif;?>
										<?}elseif(isset($arElement["SECTION"]) && ($arElement["SECTION"]["DETAIL_PICTURE"] || $arElement["SECTION"]["PICTURE"])){?>
												<?$img = CFile::GetFileArray(($arElement["SECTION"]["PICTURE"] ? $arElement["SECTION"]["PICTURE"] : $arElement["SECTION"]["DETAIL_PICTURE"]));?>
												<a href="<?=$arElement["DETAIL_PAGE_URL"]?>"><img src="<?=$img["SRC"]?>" alt="<?=$img["ALT"]?>" title="<?=$img["TITLE"]?>" /></a>
										<?}else{?>
												<a href="<?=$arElement["DETAIL_PAGE_URL"]?>"><img src="<?=\Aspro\Functions\CAsproTires2::showNoImage($arElement["IBLOCK_ID"]);?>" alt="<?=$arElement["NAME"]?>" title="<?=$arElement["NAME"]?>" /></a>
										<?}?>
									</div>
									<a href="<?=$arElement["DETAIL_PAGE_URL"]?>" class="title dark_link"><?=$name;?></a>
									...
									<?
									$totalCount = CTires2::GetTotalCount($arElement, $arParams);
									$arAddToBasketData = CTires2::GetAddToBasketArray($arElement, $totalCount, 1, $arParams["BASKET_URL"], ($bLinkedItems ? true : false), '', 'small', $arParams);
									?>
