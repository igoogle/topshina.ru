...
<?if( count( $arResult["ITEMS"] ) >= 1 ){?>
	...
	<?}?>
		...
		<?foreach($arResult["ITEMS"] as $arItem){?>
			...
			$arItemIDs=CTires2::GetItemsIDs($arItem);

			$totalCount = CTires2::GetTotalCount($arItem, $arParams);
			$arQuantityData = CTires2::GetQuantityArray($totalCount, $arItemIDs["ALL_ITEM_IDS"]);
			...
			if(!$arItem["OFFERS"] || $arParams['TYPE_SKU'] !== 'TYPE_1'){
				...
				$arAddToBasketData = CTires2::GetAddToBasketArray($arItem, $totalCount, $arParams["DEFAULT_COUNT"], $arParams["BASKET_URL"], false, $arItemIDs["ALL_ITEM_IDS"], 'small', $arParams);
			}
				...
				<table class="list_item" id="<?=$arItemIDs["strMainID"];?>">
					...
					<td class="image_block">
						<div class="image_wrapper_block">
							...
							<a href="<?=$arItem["DETAIL_PAGE_URL"]?>" class="thumb shine 123" id="<? echo $arItemIDs["ALL_ITEM_IDS"]['PICT']; ?>">
								<?
					...
					<td class="description_wrapp">
						<div class="description">
							...
							$bHideProps = (isset($arParams['VISIBLE_LIST_PROP_COUNT']) && ($arItem['OFFERS_PROPS_DISPLAY'] && count($arItem['OFFERS_PROPS_DISPLAY']) >  $arParams['VISIBLE_LIST_PROP_COUNT'] ? true : ($arItem['DISPLAY_PROPERTIES'] && count($arItem['DISPLAY_PROPERTIES']) > $arParams['VISIBLE_LIST_PROP_COUNT'] ? true : false)));
							$i = 1;
							?>
										...
										<?if ($boolShowProductProps){
											foreach( $arItem["DISPLAY_PROPERTIES"] as $arProp ){?>
												<?if( !empty( $arProp["VALUE"] ) ){?>
													<tr <?=($bHideProps && $i > $arParams['VISIBLE_LIST_PROP_COUNT'] ? 'data-hide="Y"' : '');?>>
								...
								<?if(!empty($arItem['OFFERS_PROP'])){?>
									<div class="sku_props">
										<div class="bx_catalog_item_scu wrapper_sku" id="<? echo $arItemIDs["ALL_ITEM_IDS"]['PROP_DIV']; ?>">
											<?$arSkuTemplate = array();?>
											<?$arSkuTemplate=CTires2::GetSKUPropsArray($arItem['OFFERS_PROPS_JS'], $arResult["SKU_IBLOCK_ID"], $arParams["DISPLAY_TYPE"], $arParams["OFFER_HIDE_NAME_PROPS"]);?>
										...
										<?$arItemJSParams=CTires2::GetSKUJSParams($arResult, $arParams, $arItem);?>
							...
							<?elseif($arItem["OFFERS"]):?>
								<?if(empty($arItem['OFFERS_PROP'])){?>
									<div class="offer_buy_block buys_wrapp woffers">
										...
										$arAddToBasketData = CTires2::GetAddToBasketArray($arItem, $totalCount, $arParams["DEFAULT_COUNT"], $arParams["BASKET_URL"], false, $arItemIDs["ALL_ITEM_IDS"], 'small read_more1', $arParams);?>
										<!--noindex-->
											<?=$arAddToBasketData["HTML"]?>
										<!--/noindex-->
										</div>
