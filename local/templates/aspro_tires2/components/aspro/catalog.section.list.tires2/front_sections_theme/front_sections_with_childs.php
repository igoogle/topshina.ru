...
{
	$arSections = CTires2Cache::CIBlockSection_GetList(array('SORT' => 'ASC', 'ID' => 'ASC', 'CACHE' => array('MULTI' =>'Y', 'TAG' => CTires2Cache::GetIBlockCacheTag($arParams['IBLOCK_ID']))), array('GLOBAL_ACTIVE' => 'Y', 'SECTION_ID' => $arSection['ID'], 'DEPTH_LEVEL' => 2, 'IBLOCK_ID' => $arParams['IBLOCK_ID']), $arParams['COUNT_ELEMENTS'], array('ID', 'NAME', 'SORT', 'SECTION_PAGE_URL'));
					...
					<div class="section_item_inner">
						<div class="img">
							<?if(is_array($arSection['PICTURE']) && $arSection['PICTURE']['SRC']):?>
								<?$img = CFile::ResizeImageGet($arSection['PICTURE']['ID'], array( "width" => 120, "height" => 120 ), BX_RESIZE_IMAGE_PROPORTIONAL_ALT, true );?>
							...
							<?elseif($arSection['~PICTURE']):?>
								<?$img = CFile::ResizeImageGet($arSection['~PICTURE'], array( "width" => 120, "height" => 120 ), BX_RESIZE_IMAGE_PROPORTIONAL_ALT, true );?>
								<a href="<?=$arSection['SECTION_PAGE_URL']?>" class="thumb"><img src="<?=$img['src']?>" alt="<?=$arSection['NAME']?>" title="<?=$arSection['NAME']?>" /></a>
							<?else:?>
								<a href="<?=$arSection['SECTION_PAGE_URL']?>" class="thumb"><img src="<?=\Aspro\Functions\CAsproTires2::showNoImage($arSection["IBLOCK_ID"])?>" alt="<?=$arSection['NAME']?>" title="<?=$arSection['NAME']?>" /></a>
							<?endif;?>
						</div>
