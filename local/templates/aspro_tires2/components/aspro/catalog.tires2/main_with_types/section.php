...
<?else:?>
	...
	if($arResult["VARIABLES"]["SECTION_ID"] > 0){
		$section=CTires2Cache::CIBlockSection_GetList(array('CACHE' => array("MULTI" =>"N", "TAG" => CTires2Cache::GetIBlockCacheTag($arParams["IBLOCK_ID"]))), array('GLOBAL_ACTIVE' => 'Y', "ID" => $arResult["VARIABLES"]["SECTION_ID"], "IBLOCK_ID" => $arParams["IBLOCK_ID"]), false, array("ID", "IBLOCK_ID", "NAME", "SECTION_PAGE_URL", "DESCRIPTION", "UF_SECTION_DESCR", "UF_OFFERS_TYPE", $arParams["SECTION_DISPLAY_PROPERTY"], "IBLOCK_SECTION_ID", "DEPTH_LEVEL", "LEFT_MARGIN", "RIGHT_MARGIN", "DETAIL_PICTURE", "PICTURE", "UF_TIZERS", "UF_YM_ELEMENT_ID", "UF_BANNER", "UF_GALLERY", "UF_VIDEO_YOUTUBE"));
	}
	elseif(strlen(trim($arResult["VARIABLES"]["SECTION_CODE"])) > 0){

		$section=CTires2Cache::CIBlockSection_GetList(array('CACHE' => array("MULTI" =>"N", "TAG" => CTires2Cache::GetIBlockCacheTag($arParams["IBLOCK_ID"]))), array('GLOBAL_ACTIVE' => 'Y', "=CODE" => $arResult["VARIABLES"]["SECTION_CODE"], "IBLOCK_ID" => $arParams["IBLOCK_ID"]), false, array("ID", "IBLOCK_ID", "NAME", "DESCRIPTION", "SECTION_PAGE_URL", "UF_SECTION_DESCR", "UF_OFFERS_TYPE", $arParams["SECTION_DISPLAY_PROPERTY"], "IBLOCK_SECTION_ID", "DEPTH_LEVEL", "LEFT_MARGIN", "RIGHT_MARGIN", "DETAIL_PICTURE", "PICTURE", "UF_TIZERS", "UF_YM_ELEMENT_ID", "UF_BANNER", "UF_GALLERY", "UF_VIDEO_YOUTUBE"));
	}
	...
	if($section){
		...
		$arSection["UF_TIZERS"] = $section["UF_TIZERS"];
		$arSection["UF_GALLERY"] = $section["UF_GALLERY"];
		$arSection["YM_ELEMENT_ID"] = $section["UF_YM_ELEMENT_ID"];
		$arSection["VIDEO"] = $section["~UF_VIDEO_YOUTUBE"];
		$arSection["BANNER"] = $section["UF_BANNER"];
		...
		if(strlen($section["DETAIL_PICTURE"]))
			$arSection["PICTURE"] = $section["DETAIL_PICTURE"];
		...
		} elseif(!$iSectionsCount) {
			if ($section["DEPTH_LEVEL"] > 2) {
				$sectionParent = CTires2Cache::CIBlockSection_GetList(array('CACHE' => array("MULTI" =>"N", "TAG" => CTires2Cache::GetIBlockCacheTag($arParams["IBLOCK_ID"]))), array('GLOBAL_ACTIVE' => 'Y', "ID" => $section["IBLOCK_SECTION_ID"], "IBLOCK_ID" => $arParams["IBLOCK_ID"]), false, array("ID", "IBLOCK_ID", "NAME", "UF_OFFERS_TYPE", "SECTION_PAGE_URL", "UF_TIZERS", "PICTURE", "DETAIL_PICTURE"));
				if ($sectionParent['UF_OFFERS_TYPE'] && !$typeTmpSKU) {
					$typeTmpSKU = $sectionParent['UF_OFFERS_TYPE'];
				}

				if (!$arSection["UF_TIZERS"] && $sectionParent["UF_TIZERS"]) {
					$arSection["UF_TIZERS"] = $sectionParent["UF_TIZERS"];
				}
				...
				if (!$typeTmpSKU || !$arSection["UF_TIZERS"]) {
					...
					if (!$arSection["UF_TIZERS"] && $sectionRoot["UF_TIZERS"]) {
						$arSection["UF_TIZERS"] = $sectionRoot["UF_TIZERS"];
					}
				}
			} else {
				$sectionRoot = CTires2Cache::CIBlockSection_GetList(array('CACHE' => array("MULTI" =>"N", "TAG" => CTires2Cache::GetIBlockCacheTag($arParams["IBLOCK_ID"]))), array('GLOBAL_ACTIVE' => 'Y', "<=LEFT_BORDER" => $section["LEFT_MARGIN"], ">=RIGHT_BORDER" => $section["RIGHT_MARGIN"], "DEPTH_LEVEL" => 1, "IBLOCK_ID" => $arParams["IBLOCK_ID"]), false, array("ID", "IBLOCK_ID", "NAME", "UF_OFFERS_TYPE", "SECTION_PAGE_URL", "UF_TIZERS", "PICTURE", "DETAIL_PICTURE"));
