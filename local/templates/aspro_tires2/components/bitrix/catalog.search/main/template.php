...
$componentName = (isset($arParams["COMPONENT_LIST"]) ? $arParams["COMPONENT_LIST"] : "bitrix:catalog.section");
...
if($arParams["COMPONENT_LIST"] )
{
	$arIBlocks = \Aspro\Functions\CAsproTires2::getCatalogIBlocks("", true, true);
}
else
...
<?$arElements = $APPLICATION->IncludeComponent(
	...
	Array(
		...
		"arrWHERE" => array(),
		"SHOW_WHEN" => "N",
		"PAGE_RESULT_COUNT" => $arTheme["SEARCH_RESULT_LIMIT"]["VALUE"],
		"DISPLAY_TOP_PAGER" => "N",
...
{	
	...
	?>
	<?$arItems = CTires2Cache::CIBLockElement_GetList(array('CACHE' => array("MULTI" =>"Y", "TAG" => CTires2Cache::GetIBlockCacheTag($catalogIBlockID))), array("IBLOCK_ID" => $arIBlocks, "ACTIVE"=>"Y", "IBLOCK_ACTIVE"=>"Y", "ID" => $arElements), false, false, array("ID", "IBLOCK_ID", "IBLOCK_SECTION_ID"));
	...
	if($arItems)
	{
		...
		{
			$arTmpIBlocks[$arItem["IBLOCK_ID"]] = $arItem["IBLOCK_ID"];
			...
			if($arItem["IBLOCK_SECTION_ID"])
			{
				if(is_array($arItem["IBLOCK_SECTION_ID"]))
				{
					foreach($arItem["IBLOCK_SECTION_ID"] as $id)
					{
						...
						$arAllSections[$id]["IBLOCK_ID"] = $arItem["IBLOCK_ID"];
				...
				else
				{
					...
					$arAllSections[$arItem["IBLOCK_SECTION_ID"]]["IBLOCK_ID"] = $arItem["IBLOCK_ID"];
		...
		if($arAllSections)
		{
			...
			$arSections = CTires2Cache::CIBlockSection_GetList(array('CACHE' => array("MULTI" =>"N", "GROUP" => "ID", "TAG" => CTires2Cache::GetIBlockCacheTag($catalogIBlockID))), array("ID" => $arSectionsID, "ACTIVE" => "Y", "GLOBAL_ACTIVE" => "Y", "IBLOCK_ACTIVE" => "Y", "IBLOCK_ID" => $arIBlocks), false, array("ID", "IBLOCK_ID", "NAME"));
		}?>
		<?
		$context=\Bitrix\Main\Application::getInstance()->getContext();
		$request=$context->getRequest();
		?>
		<?$setionIDRequest = ($request->get("section_id") ? $request->get("section_id") : 0);?>
		...
		<?if(($setionIDRequest || count($arTmpIBlocks) == 1) && $arTheme["FILTER_VIEW"]["VALUE"]!="HORIZONTAL"):?>
			<?ob_start()?>
				<div class="visible_mobile_filter">
					<?$APPLICATION->IncludeComponent(
						"aspro:catalog.smart.filter.tires2",
						($arParams["AJAX_FILTER_CATALOG"]=="Y" ? "main_ajax" : "main"),
						Array(
							"IBLOCK_TYPE" => $arParams["IBLOCK_TYPE"],
							"IBLOCK_ID" => ($setionIDRequest ? $arAllSections[$setionIDRequest]["IBLOCK_ID"] : reset($arTmpIBlocks)),
	...
	<?if($arTheme["FILTER_VIEW"]["VALUE"]=="HORIZONTAL"):?>
		<?if($setionIDRequest || count($arTmpIBlocks) == 1):?>
			<div class="filter_horizontal">
				<?$iblockID = ($setionIDRequest ? $arAllSections[$setionIDRequest]["IBLOCK_ID"] : reset($arTmpIBlocks))?>
				<?$APPLICATION->IncludeComponent(
					"aspro:catalog.smart.filter.tires2",
					"catalog",
					Array(
						...
						'CURRENCY_ID' => $arParams['CURRENCY_ID'],
						'DISPLAY_ELEMENT_COUNT' => $arParams['DISPLAY_ELEMENT_COUNT'],
						"INSTANT_RELOAD" => ($arParams["AJAX_FILTER_CATALOG"]=="Y" ? "Y" : "N"),
						"VIEW_MODE" => strtolower($arTheme["FILTER_VIEW"]["VALUE"]),
						"SEF_MODE" => (strlen($arResult["URL_TEMPLATES"]["smart_filter"]) ? "Y" : "N"),
						"SEF_RULE" => $arResult["FOLDER"].$arResult["URL_TEMPLATES"]["smart_filter"],
						"SMART_FILTER_PATH" => $arResult["VARIABLES"]["SMART_FILTER_PATH"],
