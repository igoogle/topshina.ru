<?
$MESS ['CT_BCSF_FILTER_TITLE'] = 'Подбор параметров';
$MESS ['CT_BCSF_FILTER_FROM'] = 'От';
$MESS ['CT_BCSF_FILTER_TO'] = 'До';
$MESS ['CT_BCSF_SET_FILTER'] = 'Подобрать';
$MESS ['CT_BCSF_DEL_FILTER'] = 'Сбросить';
$MESS ['CT_BCSF_FILTER_COUNT'] = 'Выбрано #ELEMENT_COUNT#';
$MESS ['CT_BCSF_FILTER_SHOW'] = 'Показать';
$MESS ['CT_BCSF_FILTER_ALL'] = 'Все';
$MESS ['PRICE'] = 'Цена';
$MESS ['FILTER_AUTO_TITLE'] = 'По авто';
$MESS ['FILTER_TITLE'] = 'По параметрам';
$MESS['FILTER_EXPAND_VALUES'] = 'Показать все';
$MESS['FILTER_HIDE_VALUES'] = 'Свернуть';
$MESS['IN'] = 'в ';
$MESS['ALL_HINT'] = 'Что означают эти цифры?';
$MESS['PICKING'] = 'Подбор ';
$MESS['TIRES_TAB_TITLE'] = 'Подбор шин';
$MESS['WHEELS_TAB_TITLE'] = 'Подбор дисков';
$MESS['AKB_TAB_TITLE'] = 'Подбор АКБ';
?>