...
if (!empty($arResult["ELEMENTS"]) && CModule::IncludeModule("iblock"))
{
	...
	$bHideNotAvailable = (\Bitrix\Main\Config\Option::get('aspro.tires2', 'SEARCH_HIDE_NOT_AVAILABLE', 'N') == 'Y');
	...
	while($arElement = $rsElements->Fetch())
	{
		...
		if($arOffers){
			...
			$arResult["ELEMENTS"][$arElement["ID"]]["MIN_PRICE"]=CTires2::getMinPriceFromOffersExt(
					$arOffers,
				...
				if ($arParams["SHOW_PREVIEW"] == "Y")
				{
					...
					else
					{
						if($arElement["IBLOCK_SECTION_ID"]){
							...
							{
								$arElement["PICTURE"] = CFile::ResizeImageGet($arSection['PICTURE'], array( "width" => 80, "height" => 80 ), BX_RESIZE_IMAGE_PROPORTIONAL,true );
							}
						}
					}
				}
