...
if($arUserPropValue)
{
	...
	if($arPhoneProp)
	{
		if($arUserPropValue[$arPhoneProp['ID']])
		{
			...
			{
				$mask = \Bitrix\Main\Config\Option::get('aspro.tires2', 'PHONE_MASK', '+7 (999) 999-99-99');
			...
			<script>
				...
				{
					$('input[name=ORDER_PROP_<?=$arPhoneProp['ID']?>').inputmask('mask', {'mask': arTires2Options['THEME']['PHONE_MASK']});
				})
			</script>
		<?}
	}
