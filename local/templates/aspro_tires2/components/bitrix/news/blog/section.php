...
<?if(!$arSection && $arParams['SET_STATUS_404'] !== 'Y'):?>
	...
	<?// get element start
	$arItemElementFilter = CTires2::GetCurrentElementFilter($arResult['VARIABLES'], $arParams);
	...
	$arElement = CTires2Cache::CIblockElement_GetList(array('CACHE' => array('TAG' => CTires2Cache::GetIBlockCacheTag($arParams['IBLOCK_ID']), 'MULTI' => 'N')), $arItemElementFilter, false, false, array('ID', 'PREVIEW_TEXT', 'IBLOCK_SECTION_ID', 'PREVIEW_PICTURE', 'DETAIL_PICTURE'));
	if($arElement)
	{
...
<?else:?>
	...
	{
		$arElements = CTires2Cache::CIblockElement_GetList(array('CACHE' => array('TAG' => CTires2Cache::GetIBlockCacheTag($arParams['IBLOCK_ID']), 'MULTI' => 'Y')), array_merge($arItemFilter, array("SECTION_ID" => $arItem["ID"])), false, false, array('ID', 'TAGS', 'IBLOCK_SECTION_ID'));
		if(!$arElements)
