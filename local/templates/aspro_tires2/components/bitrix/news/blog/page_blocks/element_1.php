...
<?// goods links?>
<?if(in_array('LINK_GOODS', $arParams['DETAIL_PROPERTY_CODE']) && ($arElement['PROPERTY_LINK_GOODS_VALUE'] || $arElement['PROPERTY_LINK_SECTIONS_VALUE'])):?>
	<div class="wraps goods-block with-padding block ajax_load catalog">
		...
		<?if($arElement['PROPERTY_LINK_GOODS_VALUE']):?>
			<?if($arElement['PROPERTY_LINK_SECTIONS_VALUE']):?>
				<?$GLOBALS['arrProductsFilter'][] = array(
					'LOGIC' => 'OR',
					array(
						'SECTION_ID' => $arElement['PROPERTY_LINK_SECTIONS_VALUE'],
						'INCLUDE_SUBSECTIONS' => 'Y'
					),
					array(
						'ID' => $arElement['PROPERTY_LINK_GOODS_VALUE']
					)
				);?>
			<?else:?>
				<?$GLOBALS['arrProductsFilter'] = array('ID' => $arElement['PROPERTY_LINK_GOODS_VALUE']);?>
			<?endif;?>
		<?elseif($arElement['PROPERTY_LINK_SECTIONS_VALUE']):?>
			<?$GLOBALS['arrProductsFilter']['SECTION_ID'] = $arElement['PROPERTY_LINK_SECTIONS_VALUE'];?>
		...
		if($arRegion)
		{
			if(!$arRegion["PROPERTY_SORT_REGION_PRICE_VALUE"] || $arRegion["PROPERTY_SORT_REGION_PRICE_VALUE"] == "component")
			{
				...
				$arParams["SORT_LINKED"] = "CATALOG_PRICE_".$price["ID"]; 
			}
			else
			{
				$arParams["SORT_LINKED"] = "CATALOG_PRICE_".$arRegion["PROPERTY_SORT_REGION_PRICE_VALUE"]; 
			}
		}
		else
		{
		...
		<?$APPLICATION->IncludeComponent(
			...
			array(
				...
				"SORT_BUTTONS" => ($arParams["SORT_LINKED"] ? $arParams["SORT_LINKED"] : "CATALOG_PRICE_1"),
...
<?if(in_array('LINK_SERVICES', $arParams['DETAIL_PROPERTY_CODE']) && $arElement['PROPERTY_LINK_SERVICES_VALUE']):?>
	<div class="wraps">
		...
		<?$APPLICATION->IncludeComponent("bitrix:news.list", "items-services", array(
			...
			"IBLOCK_ID" => CTires2Cache::$arIBlocks[SITE_ID]["aspro_tires2_content"]["aspro_tires2_services"][0],
			"NEWS_COUNT" => "20",
