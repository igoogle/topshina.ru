...
<div class="right_block wide_N">
	<div class="middle">
		...
		?>
		<?$arItems = CTires2Cache::CIBLockElement_GetList(array('CACHE' => array("MULTI" =>"Y", "TAG" => CTires2Cache::GetIBlockCacheTag($catalogIBlockID))), array("IBLOCK_ID" => $catalogIBlockID, "ACTIVE"=>"Y", "PROPERTY_".$arParams["LINKED_PRODUCTS_PROPERTY"] => $arElement["ID"] ), false, false, array("ID", "IBLOCK_ID", "IBLOCK_SECTION_ID"));
		...
		if($arItems)
		{
			...
			if($arAllSections)
			{
				...
				$arSections = CTires2Cache::CIBlockSection_GetList(array('CACHE' => array("MULTI" =>"N", "GROUP" => "ID", "TAG" => CTires2Cache::GetIBlockCacheTag($catalogIBlockID))), array("ID" => $arSectionsID, "IBLOCK_ID" => $catalogIBlockID), false, array("ID", "IBLOCK_ID", "NAME"));
			}
			?>
			...
			<?ob_start()?>
				<?if(count($arAllSections) > 1):?>
					<div class="top_block_filter_section">
						<div class="title"><a class="dark_link" title="<?=GetMessage("FILTER_ALL_SECTON");?>" href="<?=$APPLICATION->GetCurPageParam('', array('section_id', 'PAGEN_2'))?>"><?=GetMessage("FILTER_SECTON");?></a></div>
						<div class="items">
