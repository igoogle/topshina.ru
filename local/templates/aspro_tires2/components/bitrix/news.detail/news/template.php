...
<?if(!$bShowTopBanner && strlen($arResult['PREVIEW_TEXT'])):?>
...
<?if($bShowSales):?>
	...
	<?$APPLICATION->IncludeComponent(
		...
		array(
			...
			"IBLOCK_ID" => CTires2Cache::$arIBlocks[SITE_ID]["aspro_tires2_content"]["aspro_tires2_news"][0],
			"NEWS_COUNT" => "20",
			"SORT_BY1" => "SORT",
			"SORT_ORDER1" => "ASC",
			"SORT_BY2" => "ID",
	...
	<table class="order-block">
		<tr>
			<td class="col-md-9 col-sm-8 col-xs-7 valign">
				<?if($arResult["DISPLAY_PROPERTIES"]["PRICE"]["VALUE"]):?>
					<div class="prices_block">
						<?=\Aspro\Functions\CAsproTires2::showPrice($arResult);?>
					</div>
				<?endif;?>
				<div class="text">
					<?$APPLICATION->IncludeComponent(
			...
			<td class="col-md-3 col-sm-4 col-xs-5 valign">
				<div class="btns">
					<span class="btn btn-default btn-lg animate-load" data-event="jqm" data-param-form_id="<?=($arParams["FORM_ID_ORDER_SERVISE"] ? $arParams["FORM_ID_ORDER_SERVISE"] : 'SERVICES');?>" data-name="order_services" data-autoload-service="<?=CTires2::formatJsName($arResult['NAME']);?>" data-autoload-project="<?=CTires2::formatJsName($arResult['NAME']);?>"><span><?=(strlen($arParams['S_ORDER_SERVISE']) ? $arParams['S_ORDER_SERVISE'] : Loc::getMessage('S_ORDER_SERVISE'))?></span></span>
				</div>
			</td>
		</tr>
	</table>
