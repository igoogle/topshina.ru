...
if (!empty($arResult['ITEMS'])){
	...
	if (!empty($arResult['SKU_PROPS'])){
		$arSkuTemplate=CTires2::GetSKUPropsArray($arResult['SKU_PROPS'], $arResult["SKU_IBLOCK_ID"], "block", $arParams["OFFER_HIDE_NAME_PROPS"], "Y");
	}?>
	<script type="text/javascript">
		BX.message({
						...
						<?foreach($arResult['ITEMS'] as $key => $arItem){
							...
							elseif($arItem["OFFERS"]){
								...
								if(!$arItem['OFFERS_PROP']){
									...
									$arAddToBasketData = CTires2::GetAddToBasketArray($arItem["OFFERS"][0], $totalCount, $arParams["DEFAULT_COUNT"], $arParams["BASKET_URL"], false, $arItemIDs["ALL_ITEM_IDS"], 'small', $arParams);
								}
							}
								...
								<div class="inner_wrap">
									<div class="image_wrapper_block">
										...
										<a href="<?=$arItem["DETAIL_PAGE_URL"]?>" class="thumb shine" id="<? echo $arItemIDs["ALL_ITEM_IDS"]['PICT']; ?>">
											...
											<?else:?>
												<img border="0" src="<?=$arItem["NO_IMAGE_PATH"]?>" alt="<?=$a_alt;?>" title="<?=$a_title;?>" />
											<?endif;?>
										...
										<?if(!empty($arItem['OFFERS']) && isset($arSkuTemplate[$arItem['IBLOCK_ID']])){?>
											<?if(!empty($arItem['OFFERS_PROP'])){?>
												<div class="sku_props">
													...
													<?$arItemJSParams=CTires2::GetSKUJSParams($arResult, $arParams, $arItem, "N", "Y");?>
