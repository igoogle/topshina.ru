...
if (!empty($arResult['ITEMS'])){
	...
	foreach ($arResult['ITEMS'] as $key => $arItem)
	{
		...
		foreach($arFormatProps as $codeProp)
		{
			...
			{
				if(!ctype_digit($arItem["DISPLAY_PROPERTIES"][$codeProp]["VALUE"])){
					$arItem["DISPLAY_PROPERTIES"][$codeProp]["VALUE"] = rtrim(rtrim($arItem["DISPLAY_PROPERTIES"][$codeProp]["VALUE"], '0'), '.');
				}
			}
		}
		/**/
