...
if (!empty($arResult['ITEMS'])){
	...
	if ($arResult['MODULES']['catalog'])
	{
		...
		{
			$arSKUPropList = array();
			if($arParams['IBLOCK_IDS'])
			{
				$arSKUTmp = $arSKUPropListTmp = array();
				$offerIBlockID = '';
				foreach($arParams['IBLOCK_IDS'] as $id)
				{
					$arTmp = array();
					if($arTmp = CCatalogSKU::GetInfoByProductIBlock($id))
					{
						$arSKUTmp[] = $arTmp;
						$offerIBlockID = $arTmp["IBLOCK_ID"];
					}					
				}
				if($arSKUTmp)
				{
					foreach($arSKUTmp as $id)
					{
						$arSKUPropListTmp[] = CIBlockPriceTools::getTreeProperties(
							$id,
					...
					if($arSKUPropListTmp)
					{
						foreach($arSKUPropListTmp as $key => $arValues)
						{
							$arSKUPropList = array_merge($arValues, $arSKUPropList);
						}
					}
					$arResult["SKU_IBLOCK_ID"]=$offerIBlockID;
				}
			}
			else
			{
	...
	foreach ($arResult['ITEMS'] as $key => $arItem){
		$arGroup = CIBlockElement::GetElementGroups($arItem['ID'])->Fetch();
		if($arGroup && !$arSectionsID[$arGroup['ID']])
		{
			$arResult['ITEMS'][$key]['IBLOCK_SECTION_ID'] = $arGroup['ID'];
			$arSectionsID[$arGroup['ID']] = $arItem['IBLOCK_SECTION_ID'];
	...
	if($arSectionsID){
		$arSections = CTires2Cache::CIBlockSection_GetList(array('CACHE' => array("MULTI" =>"N", "TAG" => CTires2Cache::GetIBlockCacheTag($arIBlocksID), 'GROUP' => 'ID')), array("IBLOCK_ID" => $arIBlocksID, 'ID' => array_keys($arSectionsID)), false, array('ID', 'PICTURE'));
	...
	foreach ($arResult['ITEMS'] as $key => $arItem)
	{
		if(!$arItem['PREVIEW_PICTURE'] && !$arItem['DETAIL_PICTURE']){
			if($arSections[$arItem['IBLOCK_SECTION_ID']]['PICTURE']){
				...
				$arItem['NO_IMAGE_PATH'] = \Aspro\Functions\CAsproTires2::showNoImage($arItem["IBLOCK_ID"]);
			}
		}
		...
		{
			if ('TYPE_1' == $arParams['TYPE_SKU'] && $arParams['DISPLAY_TYPE'] !='table' && ($arParams['OFFERS_PROPERTY_CODE'] && array_intersect($arParams['OFFERS_PROPERTY_CODE'], $arSKUPropIDs)))
			{
				...
				foreach ($arItem['OFFERS'] as $keyOffer => $arOffer)
				{
					...
					if(!$arOffer['PREVIEW_PICTURE'] && !$arOffer['DETAIL_PICTURE'])
					{
						if($arSections[$arItem['IBLOCK_SECTION_ID']]['PICTURE'])
						{
							$arOffer['PREVIEW_PICTURE']['SRC'] = $arSections[$arItem['IBLOCK_SECTION_ID']]['PICTURE']['src'];
						}
						else
						{
							$arOffer['PREVIEW_PICTURE']['SRC'] = \Aspro\Functions\CAsproTires2::showNoImage($arItem["IBLOCK_ID"], $arParams['SITE_ID']);
						}
					}
				...
				foreach ($arItem['OFFERS'] as $keyOffer => $arOffer)
				{
					...
					$arAddToBasketData = CTires2::GetAddToBasketArray($arOffer, $totalCount, $arParams["DEFAULT_COUNT"], $arParams["BASKET_URL"], false, $arItemIDs["ALL_ITEM_IDS"], 'small read_more1', $arParams);
					...
					$arOneRow = array(
						...
						'AVAILIABLE' => CTires2::GetQuantityArray($totalCount),
		...
		{
			foreach ($arItem['PROPERTIES'] as $propKey => $arDispProp)
			{
				if($propKey=="CML2_ARTICLE"){
					$arItem['ARTICLE']=$arDispProp;
				}
				if ('F' == $arDispProp['PROPERTY_TYPE'])
					unset($arItem['PROPERTIES'][$propKey]);
			}
		}
