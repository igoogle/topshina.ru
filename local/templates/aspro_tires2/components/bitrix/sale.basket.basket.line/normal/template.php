...
if($bShowActual)
{
	...
	echo \Bitrix\Main\Web\Json::encode(array(
		'BASKET_SUMM' => ($arResult['NUM_PRODUCTS'] ? GetMessage("BASKET_COUNT", array("#PRICE#" => CTires2::clearFormatPrice($arResult['TOTAL_PRICE']))) : GetMessage("EMPTY_BLOCK")),
		'DELAY_SUMM' => ($sDelayPrice ? GetMessage("BASKET_DELAY_COUNT", array("#PRICE#" => CTires2::clearFormatPrice($sDelayPrice))) : GetMessage("EMPTY_BLOCK")),
		)
	);
}
else
{
	...
	usort($arBasketIDs, 'CTires2::cmpByID');?>
									...
									<tbody>
										...
										foreach($arBasketIDs as $arItem){
											if(($arItem["CAN_BUY"] == "Y") && ($arItem["DELAY"] == "N")){
												...
												$item=CTires2Cache::CIBLockElement_GetList(array('CACHE' => array("MULTI"=>"N", "TAG" => CTires2Cache::GetIBlockCacheTag($arItem["IBLOCK_ID"]))), array("ACTIVE"=>"Y", "ACTIVE_DATE" => "Y", "ID" => $arItem["PRODUCT_ID"]), false, false, array("ID", "DETAIL_PAGE_URL", "IBLOCK_ID", "PROPERTY_CML2_LINK", "PREVIEW_PICTURE", "DETAIL_PICTURE"));
												if($item["PROPERTY_CML2_LINK_VALUE"]){
													$itemLink=CTires2Cache::CIBLockElement_GetList(array('CACHE' => array("MULTI"=>"N", "TAG" => CTires2Cache::GetIBlockCacheTag($item["IBLOCK_ID"]))), array("ACTIVE"=>"Y", "ACTIVE_DATE" => "Y", "ID" => $item["PROPERTY_CML2_LINK_VALUE"]), false, false, array("ID", "PREVIEW_PICTURE", "IBLOCK_ID", "DETAIL_PICTURE"));
													if($itemLink){
														$item["ID"] = $itemLink["ID"];
