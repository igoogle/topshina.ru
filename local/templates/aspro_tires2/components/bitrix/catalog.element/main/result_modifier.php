...
if($arResult){
	$arResult['PARENT_SECTION'] = $arIBlockProperties = array();

	$arIBlockProperties = CIBlockSectionPropertyLink::GetArray($arParams['IBLOCK_ID']);

	$arResult['PARENT_SECTION'] = CTires2Cache::CIBlockSection_GetList(array('CACHE' => array("MULTI" =>"N", "TAG" => CTires2Cache::GetIBlockCacheTag($arParams["IBLOCK_ID"]))), array('GLOBAL_ACTIVE' => 'Y', "IBLOCK_ID" => $arParams["IBLOCK_ID"], '<=LEFT_BORDER' => $arResult['SECTION']['LEFT_MARGIN'], '>=RIGHT_BORDER' => $arResult['SECTION']['RIGHT_MARGIN'], 'DEPTH_LEVEL' => 1), false, array('NAME', 'PICTURE', 'SECTION_PAGE_URL', 'DESCRIPTION', 'UF_SECTION_DESCR'));
	if($arResult['PARENT_SECTION']['UF_SECTION_DESCR'])
		$arResult['PARENT_SECTION']['DESCRIPTION'] = $arResult['PARENT_SECTION']['UF_SECTION_DESCR'];
	$arResult['PARENT_SECTION']['PICTURE'] = CFile::ResizeImageGet($arResult['PARENT_SECTION']['PICTURE'], array( "width" => 150, "height" => 40 ), BX_RESIZE_IMAGE_PROPORTIONAL_ALT, true);
	if($arResult['IBLOCK_SECTION_ID'])
		$arResult['SECTION_FULL'] = CTires2Cache::CIBlockSection_GetList(array('CACHE' => array("MULTI" =>"N", "TAG" => CTires2Cache::GetIBlockCacheTag($arParams["IBLOCK_ID"]))), array('GLOBAL_ACTIVE' => 'Y', "IBLOCK_ID" => $arParams["IBLOCK_ID"], 'ID' => $arResult['IBLOCK_SECTION_ID']), false, array('ID', 'IBLOCK_ID', 'UF_VIDEO_YOUTUBE', 'DESCRIPTION'));

	if($arParams['LIST_SIMILAR_PARAMS']){
		$arResult['PARENT_SECTION']['SIMILAR_PRODUCTS_URL'] = '';

		foreach($arParams['LIST_SIMILAR_PARAMS'] as $paramName){
			if($arResult['PROPERTIES'][$paramName]['VALUE']){
				if(is_array($arResult['PROPERTIES'][$paramName]['VALUE'])){
					if($arIBlockProperties[$arResult['PROPERTIES'][$paramName]['ID']]['DISPLAY_TYPE'] == 'F'){
						foreach($arResult['PROPERTIES'][$paramName]['VALUE_ENUM_ID'] as $val){
							$arResult['PARENT_SECTION']['SIMILAR_PRODUCTS_URL'] .= '&'.$arParams['FILTER_NAME'].'_'.$arResult['PROPERTIES'][$paramName]['ID'].'_'.crc32($val).'=Y';
						}
					}
					else{
						foreach($arResult['PROPERTIES'][$paramName]['VALUE'] as $val){
							$arResult['PARENT_SECTION']['SIMILAR_PRODUCTS_URL'] .= '&'.$arParams['FILTER_NAME'].'_'.$arResult['PROPERTIES'][$paramName]['ID'].'='.crc32($val);
						}
					}
				}
				else{
					if($arIBlockProperties[$arResult['PROPERTIES'][$paramName]['ID']]['DISPLAY_TYPE'] == 'F'){
						$arResult['PARENT_SECTION']['SIMILAR_PRODUCTS_URL'] .= '&'.$arParams['FILTER_NAME'].'_'.$arResult['PROPERTIES'][$paramName]['ID'].'_'.crc32($arResult['PROPERTIES'][$paramName]['VALUE_ENUM_ID']).'=Y';
					}
					else{
						$arResult['PARENT_SECTION']['SIMILAR_PRODUCTS_URL'] .= '&'.$arParams['FILTER_NAME'].'_'.$arResult['PROPERTIES'][$paramName]['ID'].'='.crc32($arResult['PROPERTIES'][$paramName]['VALUE']);
					}
				}
...
$arResult["DISPLAY_PROPERTIES"] = \Aspro\Functions\CAsproTires2::getFormatedTIProps($arResult["DISPLAY_PROPERTIES"]);

/* remove .000 from value */
foreach($arFormatProps as $codeProp)
{
	if($arResult["PROPERTIES"][$codeProp]["VALUE"])
	{
		if(!ctype_digit($arResult["PROPERTIES"][$codeProp]["VALUE"])){
			$arResult["PROPERTIES"][$codeProp]["VALUE"] = rtrim(rtrim($arResult["PROPERTIES"][$codeProp]["VALUE"], '0'), '.');
		}
	}
	if($arResult["DISPLAY_PROPERTIES"][$codeProp]["VALUE"])
	{
		if(!ctype_digit($arResult["DISPLAY_PROPERTIES"][$codeProp]["VALUE"])){
			$arResult["DISPLAY_PROPERTIES"][$codeProp]["DISPLAY_VALUE"] = rtrim(rtrim($arResult["DISPLAY_PROPERTIES"][$codeProp]["VALUE"], '0'), '.');
		}
	}
}
/**/
...
$arSectionImage = array();

if($arResult && !$arResult['DETAIL_PICTURE'] && $arResult['SECTION']['PICTURE']){
	//$arSectionImage = CFile::ResizeImageGet($arResult['SECTION']['PICTURE'], array( "width" => 400, "height" => 400 ), BX_RESIZE_IMAGE_PROPORTIONAL_ALT,true);
}

if($arResult && !$arResult['DETAIL_PICTURE'] && !$arSectionImage){
	$strEmptyPreview = \Aspro\Functions\CAsproTires2::showNoImage($arResult["IBLOCK_ID"]);
}

if ($strEmptyPreview && file_exists($_SERVER['DOCUMENT_ROOT'].$strEmptyPreview))
{
...
$productSlider = CTires2::getSliderForItemExt($arResult, $arParams['ADD_PICT_PROP'], 'Y' == $arParams['ADD_DETAIL_TO_SLIDER'], true, 'Y' == $arParams['ADD_SECTION_DETAIL_TO_SLIDER'], $arParams['ORDER_DETAIL_SECTION']);
...
if(empty($productSlider))
{
	...
	/*elseif($arResult['SECTION']['PICTURE']){
		$arSectionImage['SRC'] = $arSectionImage['src'];
		$productSlider = array(
			0 => array(
				'ID' => $arResult['SECTION']['PICTURE']
			),
		);
	}*/
	else{
	...
	foreach($productSlider as $i => $arImage){
		$productSlider[$i] = array_merge(
			$arImage, array(
				...
				"SMALL" => CFile::ResizeImageGet($arImage["ID"], array("width" => 500, "height" => 500), BX_RESIZE_IMAGE_PROPORTIONAL_ALT, true, array()),
				"THUMB" => CFile::ResizeImageGet($arImage["ID"], array("width" => 50, "height" => 50), BX_RESIZE_IMAGE_PROPORTIONAL_ALT, true, array()),
			)
		);
	}
}
...
$arPropsHint = \Aspro\Functions\CAsproTires2::getPropsHint($arParams);
$arResult["TIRES_PROP"] = \Aspro\Functions\CAsproTires2::getPropsHintValue($arResult, $arPropsHint);
			...
			if($arOffer['PROPERTIES'] && isset($arOffer['PROPERTIES'][$arParams['ADDITIONAL_GALLERY_OFFERS_PROPERTY_CODE']]) && $arOffer['PROPERTIES'][$arParams['ADDITIONAL_GALLERY_OFFERS_PROPERTY_CODE']]['VALUE']){
				foreach($arOffer['PROPERTIES'][$arParams['ADDITIONAL_GALLERY_OFFERS_PROPERTY_CODE']]['VALUE'] as $img){
					...
					$arPhoto = array(
						...
						'THUMB' => CFile::ResizeImageGet($img , array('width' => 60, 'height' => 60), BX_RESIZE_IMAGE_PROPORTIONAL_ALT, true),
						'TITLE' => $title,
						'ALT' => $alt,
					);
					if('TYPE_1' === $arParams['TYPE_SKU']){
...
if ($arResult['CATALOG'] && isset($arResult['OFFERS']) && !empty($arResult['OFFERS']))
{
	...
	foreach ($arResult['OFFERS'] as $keyOffer => $arOffer)
	{
		...
		$offerSlider = CTires2::getSliderForItemExt($arOffer, $arParams['OFFER_ADD_PICT_PROP'], true); // $arParams['ADD_DETAIL_TO_SLIDER'] == 'Y'
		...
		if($arOffer['MORE_PHOTO']){
			foreach($arOffer['MORE_PHOTO'] as $i => $arImage){
				if($arImage["ID"]){
					...
					$arOffer['MORE_PHOTO'][$i]["SMALL"] = CFile::ResizeImageGet($arImage["ID"], array("width" => 500, "height" => 500), BX_RESIZE_IMAGE_PROPORTIONAL_ALT, true, array());
					$arOffer['MORE_PHOTO'][$i]["THUMB"] = CFile::ResizeImageGet($arImage["ID"], array("width" => 52, "height" => 52), BX_RESIZE_IMAGE_PROPORTIONAL_ALT, true, array());
				}
			}
		}
	...
	$postfix = '';
	global $arSite;
	if(\Bitrix\Main\Config\Option::get("aspro.tires2", "HIDE_SITE_NAME_TITLE", "N")=="N")
	...
	if('TYPE_1' == $arParams['TYPE_SKU'] && $arResult['OFFERS'] ){
		foreach ($arResult['OFFERS'] as $keyOffer => $arOffer)
		{
			...
			if (!empty($arOffer['DISPLAY_PROPERTIES']))
			{
				$boolSKUDisplayProps = true;
				$arSKUProps = array();
				$arOffer["DISPLAY_PROPERTIES"] = \Aspro\Functions\CAsproTires2::prepareItemProps($arOffer["DISPLAY_PROPERTIES"]);
			...
			$totalCount = CTires2::GetTotalCount($arOffer, $arParams);
			$arOffer['IS_OFFER'] = 'Y';
			...
			if($arParams['USE_PRICE_COUNT'] == 'Y')
			{
				...
				$arOffer = array_merge($arOffer, CTires2::formatPriceMatrix($arOffer));
				$sPriceMatrix = CTires2::showPriceMatrix($arOffer, $arParams, $arOffer['~CATALOG_MEASURE_NAME']);
			}

			$arAddToBasketData = CTires2::GetAddToBasketArray($arOffer, $totalCount, $arParams["DEFAULT_COUNT"], $arParams["BASKET_URL"], false, $arItemIDs["ALL_ITEM_IDS"], 'btn-lg w_icons', $arParams);
			...
			$arOneRow = array(
				...
				'AVAILIABLE' => CTires2::GetQuantityArray($totalCount, array(), "Y"),
	...
	/*set min_price_id*/
	if('TYPE_1' != $arParams['TYPE_SKU'] && $arResult['OFFERS'] ){
		$arResult['MIN_PRICE'] = CTires2::getMinPriceFromOffersExt(
			$arResult['OFFERS'],
		...
		foreach ($arResult['OFFERS'] as $keyOffer => $arOffer){
			...
			if($arParams['USE_PRICE_COUNT'] == 'Y')
			{
				...
				$arResult['OFFERS'][$keyOffer] = array_merge($arOffer, CTires2::formatPriceMatrix($arOffer));
			}
		}
	...
	if($arParams['USE_PRICE_COUNT'] == 'Y')
	{
		if($arResult['OFFERS'])
		{
			foreach($arResult['OFFERS'] as $keyOffer => $arOffer)
			{
				...
				$arOffer["FIX_PRICE_MATRIX"] = CTires2::checkPriceRangeExt($arOffer);
				$arResult['OFFERS'][$keyOffer] = array_merge($arOffer, CTires2::formatPriceMatrix($arOffer));
			}
			$arResult['MIN_PRICE'] = CTires2::getMinPriceFromOffersExt(
				$arResult['OFFERS'],
		...
		{
			$arResult["FIX_PRICE_MATRIX"] = CTires2::checkPriceRangeExt($arResult);
		}
	}

	//format prices when USE_PRICE_COUNT
	$arResult = array_merge($arResult, CTires2::formatPriceMatrix($arResult));
}
...
if (!empty($arResult['DISPLAY_PROPERTIES'])){
	...
	foreach ($arResult['DISPLAY_PROPERTIES'] as $propKey => $arDispProp){
		if($arDispProp['VALUE']){
			if($arDispProp["CODE"] == $arTheme["PROP_TYRE_SEASON"]["VALUE"] || $arDispProp["CODE"] == $arTheme['PROP_TYRE_SEASON_MOTO']['VALUE'] || $arDispProp["CODE"] == $arTheme['PROP_TYRE_SEASON_TRUCK']['VALUE']){
				if($arDispProp["VALUE"] == $arTheme["PROP_TYRE_SEASON_SUMMER"]["VALUE"])
					$arDispProp["EXT_CLASS"] = "icon-summer";
				if($arDispProp["VALUE"] == $arTheme["PROP_TYRE_SEASON_WINTER"]["VALUE"])
					$arDispProp["EXT_CLASS"] = "icon-winter";
				if($arDispProp["VALUE"] == $arTheme["PROP_TYRE_SEASON_ALL"]["VALUE"])
					$arDispProp["EXT_CLASS"] = "icon-allseason";
			}

			if($arDispProp["CODE"] == $arTheme["PROP_TYRE_SPIKES"]["VALUE"] || $arDispProp["CODE"] == $arTheme["PROP_TYRE_SPIKES_MOTO"]["VALUE"] || $arDispProp["CODE"] == $arTheme["PROP_TYRE_SPIKES_TRUCK"]["VALUE"]){
				if($arDispProp["VALUE"] == $arTheme["PROP_TYRE_SPIKES_Y"]["VALUE"])
					$arDispProp["EXT_CLASS"] = "icon-spikes";
				if($arDispProp["VALUE"] == $arTheme["PROP_TYRE_SPIKES_N"]["VALUE"])
					$arDispProp["EXT_CLASS"] = "icon-wthspikes";
			}

			if($arDispProp["CODE"] == $arTheme["PROP_TYRE_RNF"]["VALUE"]){
				$arDispProp["DISPLAY_VALUE"]=$arDispProp["NAME"];

				if($arDispProp["CODE"] == $arTheme["PROP_TYRE_RNF"]["VALUE"] && (($arTheme["PROP_TYRE_RNF_ONE"]["VALUE"] != "N" && $arDispProp["VALUE"] == $arTheme["PROP_TYRE_RNF_ONE"]["DEPENDENT_PARAMS"]["PROP_TYRE_RNF_Y"]["VALUE"]) || $arTheme["PROP_TYRE_RNF_ONE"]["VALUE"] == "N")){
					$arDispProp["EXT_CLASS"] = "icon-rnf";
				}
			}
		}
...
/*akc*/
if((int)$arParams["IBLOCK_STOCK_ID"])
{
	$arSelect = array("ID", "IBLOCK_ID", "IBLOCK_SECTION_ID", "NAME", "PREVIEW_PICTURE", "PREVIEW_TEXT", "DETAIL_PAGE_URL");
	$arResult["STOCK"] = CTires2Cache::CIBLockElement_GetList(array('CACHE' => array("TAG" => CTires2Cache::GetIBlockCacheTag($arParams["IBLOCK_STOCK_ID"]))), array("IBLOCK_ID" => $arParams["IBLOCK_STOCK_ID"], "ACTIVE"=>"Y", "ACTIVE_DATE" => "Y", "PROPERTY_LINK_GOODS" => $arResult["ID"]), false, false, $arSelect);
}

/*brand item*/
$arBrand = array();
if($arResult["PROPERTIES"]["BRAND"]["LINK_IBLOCK_ID"])
{
	if(strlen($arResult["DISPLAY_PROPERTIES"]["BRAND"]["VALUE"]))
	{
		$arBrand = CTires2Cache::CIBLockElement_GetList(array('CACHE' => array("MULTI" =>"N", "TAG" => CTires2Cache::GetIBlockCacheTag($arResult["PROPERTIES"]["BRAND"]["LINK_IBLOCK_ID"]))), array("IBLOCK_ID" => $arResult["PROPERTIES"]["BRAND"]["LINK_IBLOCK_ID"], "ACTIVE"=>"Y", "ID" => $arResult["DISPLAY_PROPERTIES"]["BRAND"]["VALUE"]), false, false, array("ID", "NAME", "DETAIL_PAGE_URL", "PREVIEW_PICTURE", "PREVIEW_TEXT"));
		if($arBrand)
		{
			...
			$arResult["PARENT_SECTION"] = array(
				"PICTURE" => $arBrand["IMAGE"],
				"NAME" => $arBrand["NAME"],
				"SECTION_PAGE_URL" => $arBrand["DETAIL_PAGE_URL"],
				"DESCRIPTION" => $arBrand["PREVIEW_TEXT"],
			);
		}
	}
	else
...
/*stores product*/
$arStores=CTires2Cache::CCatalogStore_GetList(array(), array("ACTIVE" => "Y"), false, false, array());
$arResult["STORES_IDS"] = array();
if($arStores)
{
	foreach($arStores as $arStore)
		$arResult["STORES_IDS"][] = $arStore["ID"];
}
$arResult["STORES_COUNT"] = count($arStores);

$arResult["GROUPS_PROPS"] = \Aspro\Functions\CAsproTires2::prepareItemProps($arResult["DISPLAY_PROPERTIES"]);

/*get section table sizes*/
$tmpTableSizes = $tableSizes = '';
if($arResult["SECTION"])
{
	$arSection = CTires2Cache::CIBlockSection_GetList(array('CACHE' => array("MULTI" =>"N", "TAG" => CTires2Cache::GetIBlockCacheTag($arParams["IBLOCK_ID"]))), array('GLOBAL_ACTIVE' => 'Y', "ID" => $arResult["SECTION"]["ID"], "IBLOCK_ID" => $arParams["IBLOCK_ID"]), false, array("ID", "IBLOCK_ID", "LEFT_MARGIN", "RIGHT_MARGIN", "NAME", "UF_TABLE_SIZES", "DESCRIPTION"));
...
/*get tizers section*/
if(is_array($arParams["SECTION_TIZER"]) && $arParams["SECTION_TIZER"] && \Bitrix\Main\Loader::includeModule('highloadblock')){
	$arTizersData = array();
