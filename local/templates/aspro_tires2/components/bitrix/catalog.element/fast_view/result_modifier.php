...
$arResult['PARENT_SECTION'] = CTires2Cache::CIBlockSection_GetList(array('CACHE' => array("MULTI" =>"N", "TAG" => CTires2Cache::GetIBlockCacheTag($arParams["IBLOCK_ID"]))), array('GLOBAL_ACTIVE' => 'Y', "IBLOCK_ID" => $arParams["IBLOCK_ID"], '<=LEFT_BORDER' => $arResult['SECTION']['LEFT_MARGIN'], '>=RIGHT_BORDER' => $arResult['SECTION']['RIGHT_MARGIN'], 'DEPTH_LEVEL' => 1), false, array('NAME', 'PICTURE', 'SECTION_PAGE_URL', 'DESCRIPTION'));
...
// $productSlider = CTires2::getSliderForItemExt($arResult, $arParams['ADD_PICT_PROP'], 'Y' == $arParams['ADD_DETAIL_TO_SLIDER']);
...
/*elseif($arResult['SECTION']['PICTURE'] && in_array($arResult["IBLOCK_ID"], \Aspro\Functions\CAsproTires2::getCatalogIBlocks()))
	$productSlider[] = CFile::GetFileArray($arResult['SECTION']['PICTURE']);*/
...
if ($arResult['CATALOG'] && isset($arResult['OFFERS']) && !empty($arResult['OFFERS']))
{
	...
	if('TYPE_1' == $arParams['TYPE_SKU'] && $arResult['OFFERS'] ){
		foreach ($arResult['OFFERS'] as $keyOffer => $arOffer)
		{
			...
			$arAddToBasketData = CTires2::GetAddToBasketArray($arOffer, $totalCount, $arParams["DEFAULT_COUNT"], $arParams["BASKET_URL"], false, $arItemIDs["ALL_ITEM_IDS"], 'w_icons', $arParams);
...
if (!empty($arResult['DISPLAY_PROPERTIES'])){
	foreach ($arResult['DISPLAY_PROPERTIES'] as $propKey => $arDispProp){
		if(('F' == $arDispProp['PROPERTY_TYPE'] || (in_array($arDispProp["CODE"], \Aspro\Functions\CAsproTires2::$arHideProps))) && !$arDispProp['VALUE'])
			unset($arResult['DISPLAY_PROPERTIES'][$propKey]);
	}
	foreach ($arResult['DISPLAY_PROPERTIES'] as $propKey => $arDispProp){
		if(!in_array($arDispProp["CODE"], \Aspro\Functions\CAsproTires2::$arHideProps))
