...
if($arResult["OFFERS"]){
	...
	$arAddToBasketData = CTires2::GetAddToBasketArray($arResult, $totalCount, $arParams["DEFAULT_COUNT"], $arParams["BASKET_URL"], false, $arItemIDs["ALL_ITEM_IDS"], 'big_btn w_icons btn-lg', $arParams);
}
				...
				<?if(!$arResult["OFFERS"]):?>
					<div class="counter_wrapp">
						<?if(($arAddToBasketData["OPTIONS"]["USE_PRODUCT_QUANTITY_DETAIL"] && $arAddToBasketData["ACTION"] == "ADD") && $arAddToBasketData["CAN_BUY"]):?>
							<div class="counter_block big" data-offers="<?=($arResult["OFFERS"] ? "Y" : "N");?>" data-item="<?=$arResult["ID"];?>" <?=(($arResult["OFFERS"] && $arParams["TYPE_SKU"]=="N") ? "style='display: none;'" : "");?>>
					...
					<?if($arAddToBasketData["ACTION"] !== "NOTHING"):?>
						<?if($arAddToBasketData["ACTION"] == "ADD" && $arAddToBasketData["CAN_BUY"] && $arParams["SHOW_ONE_CLICK_BUY"]!="N"):?>
							<div class="wrapp_one_click">
								<span class="btn btn-default white one_click btn-lg" data-item="<?=$arResult["ID"]?>" data-iblockID="<?=$arParams["IBLOCK_ID"]?>" data-quantity="<?=$arAddToBasketData["MIN_QUANTITY_BUY"];?>" onclick="oneClickBuy('<?=$arResult["ID"]?>', '<?=$arParams["IBLOCK_ID"]?>', this)">
	...
	<div class="right_info">
		<div class="info_item scrollbar">
			<div class="title hidden">
				<a href="<?=$arResult["DETAIL_PAGE_URL"];?>" class="dark_link"><?=$elementName;?></a>
			...
			<div class="top_info">
				<div class="rows_block">
					...
					<?endif;?>
					<div class="item_block status">
						<?=$arQuantityData["HTML"];?>
					</div>
					<?//if($arResult["ARTICLE"]):?>
						<div class="item_block art">
							<div class="article iblock" itemprop="additionalProperty" itemscope itemtype="http://schema.org/PropertyValue" <?if($arResult['SHOW_OFFERS_PROPS']){?>id="<? echo $arItemIDs["ALL_ITEM_IDS"]['DISPLAY_PROP_ARTICLE_DIV'] ?>" style="display: none;"<?}?>>
								<span class="block_title" itemprop="name"><?=($arResult["ARTICLE"] ? GetMessage('ARTICLE_TITLE')." " : "");?></span>
				...
				<?if($arParams["SHOW_CHEAPER_FORM"] == "Y"):?>
					<div class="quantity_block_wrapper">
						<div class="cheaper_form">
							<span class="animate-load" data-event="jqm" data-param-form_id="CHEAPER" data-name="cheaper" data-autoload-product_name="<?=CTires2::formatJsName($arResult["NAME"]);?>" data-autoload-product_id="<?=$arResult["ID"];?>"><?=($arParams["CHEAPER_FORM_NAME"] ? $arParams["CHEAPER_FORM_NAME"] : GetMessage("CHEAPER"));?></span>
						</div>
					</div>
				<?endif;?>
			</div>
			...
			<div class="iblock char_block" <?=(!$arResult["DISPLAY_PROPERTIES"] ? 'style="display:none;"' : '');?>>
				...
				<?else:?>
					<?if($arResult['GROUPS_PROPS']):?>
						<?$nCountProps = count($arResult['GROUPS_PROPS']);?>
						<?$i = 1;?>
						<table class="props_list items">
							<?foreach($arResult['GROUPS_PROPS'] as $arProp):?>
								<tr class="<?=($i++ > $arParams['VISIBLE_PROP_COUNT'] ? 'hidden' : '');?>" itemprop="additionalProperty" itemscope itemtype="http://schema.org/PropertyValue">
									<td class="char_name">
						...
						<table class="props_list" id="<? echo $arItemIDs["ALL_ITEM_IDS"]['DISPLAY_PROP_DIV']; ?>"></table>
						<?if($nCountProps > $arParams['VISIBLE_PROP_COUNT']):?>
							<div class="more_wrapper"><span class="colored more"><?=\Bitrix\Main\Localization\Loc::getMessage("ALL_CHARS");?></span></div>
						<?endif;?>
					<?endif;?>
				<?endif;?>
			</div>
		...
		<?if($arResult['PARENT_SECTION']):?>
			...
			<div class="brand_filter">
				<?if($arResult['PARENT_SECTION']['PICTURE']):?>
					<div class="img_brand"><a href="<?=$arResult['PARENT_SECTION']['SECTION_PAGE_URL']?>" title="<?=$arResult['PARENT_SECTION']['NAME']?>"><img src="<?=$arResult['PARENT_SECTION']['PICTURE']['src']?>" alt="<?=$arResult['PARENT_SECTION']['NAME']?>" /></a></div>
				<?endif;?>
				<?$allURL = ($arResult['BRAND_ITEM'] ? $arResult['PARENT_SECTION']['SECTION_PAGE_URL'] : $arTheme['CATALOG_SEARCH_PAGE_URL']['VALUE'].'?q='.$arResult['PARENT_SECTION']['NAME']);?>
				<div><a href="<?=$allURL;?>" title="<?=$arResult['PARENT_SECTION']['NAME']?>"><?=\Bitrix\Main\Localization\Loc::getMessage("ALL_ITEMS_BRAND", array( "#BRAND_NAME#" => $arResult['PARENT_SECTION']['NAME']));?></a></div>
			</div>
		<?endif;?>
	</div>
	<?/*
