<?
	...
	if($bStoreBuy)
	{
		$dbRes = CCatalogStore::GetList(array(), array('ACTIVE' => 'Y'), false, false, array('ID', 'TITLE', 'ADDRESS'));
...
<div class="module-cart">
		...
		<table class="colored" width="100%" id="basket_items">
			...
			<tbody>
				<?foreach ($arResult["GRID"]["ROWS"] as $k => $arItem):
					...
					if($arItem["PROPS"])
					{
						foreach($arItem["PROPS"] as $arTmpProp)
						{
							if($arTmpProp["CODE"] == "STORES")
								$storeTitleProp = $arTmpProp["VALUE"];
						}
					}
					...
					if($arStores)
					{
						foreach($arStores as $arStore)
						{
							...
							if($storeTitle == $storeTitleProp)
								$storeID = $arStore["ID"];
						}
					}
					if($storeID)
					{
						$arStoreProduct = CCatalogStore::GetList(array('TITLE' => 'ASC', 'ID' => 'ASC'), array("ACTIVE" => "Y", "PRODUCT_ID" => $arItem['PRODUCT_ID'], "ID" => $storeID), false, false, array("ID", "PRODUCT_AMOUNT"))->Fetch();
							$arItem["AVAILABLE_QUANTITY"] = $arStoreProduct["PRODUCT_AMOUNT"];
					}
					?>
							...
							?>
								<td class="thumb-cell">
									...
									<?}elseif( strlen($arItem["DETAIL_PICTURE"]["SRC"])>0 ){?>
										...
										<?if (strlen($arItem["DETAIL_PAGE_URL"]) > 0):?></a><?endif;?>
									<?}elseif(strlen($arItem["PICTURE"])>0){?>
										<?if (strlen($arItem["DETAIL_PAGE_URL"]) > 0):?><a href="<?=$arItem["DETAIL_PAGE_URL"]?>" class="thumb"><?endif;?>
											<img src="<?=$arItem["PICTURE"];?>" alt="<?=$arItem["NAME"]?>" title="<?=$arItem["NAME"]?>" width="70" height="70" />
										<?if (strlen($arItem["DETAIL_PAGE_URL"]) > 0):?></a><?endif;?>
									<?}else{?>
										<?if (strlen($arItem["DETAIL_PAGE_URL"]) > 0):?><a href="<?=$arItem["DETAIL_PAGE_URL"]?>" class="thumb"><?endif;?>
											<img src="<?=$arItem['NO_IMAGE_URL'];?>" alt="<?=$arItem["NAME"]?>" title="<?=$arItem["NAME"]?>" width="70" height="70" />
										<?if (strlen($arItem["DETAIL_PAGE_URL"]) > 0):?></a><?endif;?>
									<?}?>
								...
								<td class="name-cell" style="padding-left:0; padding-right:0;">
									...
									<?if($storeTitleProp):?>
										<div class="store_title" style="display:none;"><?=$arTmpProp["VALUE"]?></div>
									<?endif;?>
									<?if ($bPropsColumn):?>
	...
	<?$arError = CTires2::checkAllowDelivery($arResult["allSum"],$currency);?>
	<div class="itog">
		<table class="colored fixed" height="100%" width="100%">
			...
			<tfoot>
				...
				<tr data-id="total_buttons" class="bottom_btn">
					...
					<?endif;?>
					<td class="back_btn">
						<?if(!$arError["ERROR"] && \Bitrix\Main\Config\Option::get("aspro.tires2", "SHOW_ONECLICKBUY_ON_BASKET_PAGE", "Y") == "Y"){?>
					...
					<td width="19%">
						<?if(!$arError["ERROR"]){?>
							<?if (\Bitrix\Main\Config\Option::get("aspro.tires2", "SHOW_ONECLICKBUY_ON_BASKET_PAGE", "Y") == "Y"):?>
