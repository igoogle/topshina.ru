							...
							<?if($bShowAskBlock || $bShowOrderBlock):?>
								<div class="col-md-<?=($arResult['DISPLAY_PROPERTIES_FORMATTED'] ? 6 : 12)?>">
									<div class="buttons-block">
										<?if($bShowOrderBlock):?>
											<div class="block">
												<span class="btn btn-default btn-lg animate-load" data-event="jqm" data-param-form_id="<?=($arParams["FORM_ID_ORDER_SERVISE"] ? $arParams["FORM_ID_ORDER_SERVISE"] : 'PROJECTS');?>" data-name="order_services" data-autoload-service="<?=CTires2::formatJsName($arResult['NAME']);?>" data-autoload-project="<?=CTires2::formatJsName($arResult['NAME']);?>"><span><?=(strlen($arParams['S_ORDER_SERVISE']) ? $arParams['S_ORDER_SERVISE'] : Loc::getMessage('S_ORDER_SERVISE'))?></span></span>
											</div>
										<?endif;?>
										<?if($bShowAskBlock):?>
											<div class="block">
...
<?if($arResult['DISPLAY_PROPERTIES']['FORM_QUESTION']['VALUE_XML_ID'] == 'YES'):?>
	...
	<?endif;?>
			<div class="ask_a_question">
				...
				<div class="outer">
					<span><span class="btn btn-default btn-lg white animate-load" data-event="jqm" data-param-form_id="ASK" data-autoload-need_product="<?=CTires2::formatJsName($arResult['NAME']);?>" data-name="question"><span><?=(strlen($arParams['S_ASK_QUESTION']) ? $arParams['S_ASK_QUESTION'] : Loc::getMessage('S_ASK_QUESTION'))?></span></span></span>
				</div>
			</div>
	<?if($isMenu):?>
		<?$this->EndViewTarget();?>
