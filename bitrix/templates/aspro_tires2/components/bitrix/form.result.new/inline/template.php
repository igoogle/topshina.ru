...
<div class="form inline <?=$arResult["arForm"]["SID"]?>">
	...
	<?if($arResult["isFormErrors"] == "Y" || strlen($arResult["FORM_NOTE"])):?>
		<div class="form_result <?=($arResult["isFormErrors"] == "Y" ? 'error' : 'success')?>">
			...
			<?else:?>
				...
				<?endif;?>
				<script>
					if(arTires2Options['THEME']['USE_FORMS_GOALS'] !== 'NONE')
					{
						var eventdata = {goal: 'goal_webform_success' + (arTires2Options['THEME']['USE_FORMS_GOALS'] === 'COMMON' ? '' : '_<?=$arResult["arForm"]["ID"]?>')};
	...
	<?if($arParams['HIDE_SUCCESS'] != 'Y' || ($arParams['HIDE_SUCCESS'] == 'Y' && !strlen($arResult["FORM_NOTE"]))):?>
		...
		<div class="form_body">
			<?if(is_array($arResult["QUESTIONS"])):?>
				<?if(!$bLeftAndRight):?>
					<?foreach($arResult["QUESTIONS"] as $FIELD_SID => $arQuestion):?>
						<?CTires2::drawFormField($FIELD_SID, $arQuestion);?>
					<?endforeach;?>
				<?else:?>
					<div class="row">
						<div class="col-md-7">
			...
			<?$bHiddenCaptcha = (isset($arParams["HIDDEN_CAPTCHA"]) ? $arParams["HIDDEN_CAPTCHA"] : COption::GetOptionString("aspro.tires2", "HIDDEN_CAPTCHA", "Y"));?>
		...
		<div class="form_footer">
			<?$bShowLicenses = (isset($arParams["SHOW_LICENCE"]) ? $arParams["SHOW_LICENCE"] : COption::GetOptionString("aspro.tires2", "SHOW_LICENCE", "Y"));?>
			<?if($bShowLicenses == "Y"):?>
				<div class="licence_block filter label_block">
					<input type="checkbox" id="licenses_inline" <?=(COption::GetOptionString("aspro.tires2", "LICENCE_CHECKED", "N") == "Y" ? "checked" : "");?> name="licenses_inline" required value="Y">
			...
			$(document).ready(function(){
				...
				if(arTires2Options['THEME']['PHONE_MASK'].length){
					...
					$('form[name=<?=$arResult["arForm"]["VARNAME"]?>] input.phone, form[name=<?=$arResult["arForm"]["VARNAME"]?>] input[data-sid=PHONE]').inputmask('mask', {'mask': arTires2Options['THEME']['PHONE_MASK'] });
