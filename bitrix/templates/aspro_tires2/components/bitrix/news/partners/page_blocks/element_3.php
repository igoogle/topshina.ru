...
<?if($arParams["SHOW_LINKED_PRODUCTS"] == "Y" && strlen($arParams["LINKED_PRODUCTS_PROPERTY"])):?>
	<?global $arTheme?>
	<?$arItems = CTires2Cache::CIBLockElement_GetList(array('CACHE' => array("MULTI" =>"Y", "TAG" => CTires2Cache::GetIBlockCacheTag($arParams["IBLOCK_CATALOG_ID"]))), array("IBLOCK_ID" => $arParams["IBLOCK_CATALOG_ID"], "ACTIVE"=>"Y", "PROPERTY_".$arParams["LINKED_PRODUCTS_PROPERTY"] => $arElement["ID"] ), false, false, array("ID", "IBLOCK_ID", "IBLOCK_SECTION_ID"));
	if($arItems)
	{
		$arSectionsID = array();
		...
		if($arSectionsID):?>
			<div class="wraps goods-block with-padding">
				...
				<?$APPLICATION->IncludeComponent(
					"aspro:catalog.section.list.tires2",
					"front_sections_only",
					array(
