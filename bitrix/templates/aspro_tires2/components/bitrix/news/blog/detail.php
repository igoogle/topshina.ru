...
$arElement = CTires2Cache::CIblockElement_GetList(array('CACHE' => array('TAG' => CTires2Cache::GetIBlockCacheTag($arParams['IBLOCK_ID']), 'MULTI' => 'N')), $arItemFilter, false, false, array('ID', 'PREVIEW_TEXT', 'IBLOCK_SECTION_ID', 'PREVIEW_PICTURE', 'DETAIL_PICTURE', 'PROPERTY_LINK_GOODS', 'PROPERTY_LINK_SECTIONS', 'PROPERTY_LINK_REGION'));
...
<?else:?>
	...
	<div class="detail <?=($templateName = $component->{'__template'}->{'__name'})?>">
		...
		?>
		<?$arSections = CTires2Cache::CIBLockSection_GetList(array('SORT' => 'ASC', 'NAME' => 'ASC', 'CACHE' => array('TAG' => CTires2Cache::GetIBlockCacheTag($arParams['IBLOCK_ID']), 'GROUP' => array('ID'), 'MULTI' => 'N', 'URL_TEMPLATE' => $arResult['FOLDER'].$arResult['URL_TEMPLATES']['section'])), array('IBLOCK_ID' => $arParams['IBLOCK_ID'], 'DEPTH_LEVEL' => 1, 'ACTIVE' => 'Y'), false, array('ID', 'SECTION_PAGE_URL'));
		$arTags = array();
		if($arSections)
		{
			foreach($arSections as $key => $arSection)
			{
				$arElements = CTires2Cache::CIblockElement_GetList(array('CACHE' => array('TAG' => CTires2Cache::GetIBlockCacheTag($arParams['IBLOCK_ID']), 'MULTI' => 'Y')), array_merge($arItemFilter, array("SECTION_ID" => $arSection["ID"])), false, false, array('ID', 'TAGS'));
				if(!$arElements)
		...
		{
			$arElements = CTires2Cache::CIblockElement_GetList(array('CACHE' => array('TAG' => CTires2Cache::GetIBlockCacheTag($arParams['IBLOCK_ID']), 'MULTI' => 'Y')), $arItemFilter, false, false, array('ID', 'TAGS'));
		...
		<?$this->__component->__template->SetViewTarget('under_sidebar_content');?>
			...
			<?if($arParams['ALSO_ITEMS_POSITION'] == 'side'):?>
				<?$APPLICATION->IncludeComponent("bitrix:news.list", "items-blog-list", array(
					...
					"IBLOCK_ID" => CTires2Cache::$arIBlocks[SITE_ID]["aspro_tires2_content"]["aspro_tires2_articles"][0],
				...
				<?$APPLICATION->IncludeComponent(
					...
					Array(
						...
						"WIDTH" => "100%",
						"arrFILTER" => array("iblock_aspro_tires2_content"),
						"arrFILTER_iblock_aspro_tires2_content" => array($arParams["IBLOCK_ID"])
					), $component
				);?>
			<?endif;?>
