...
$arItems=CTires2::getBasketItems();?>
<script>
	touchItemBlock('.catalog_item a');
	<?if(is_array($arItems["DELAY"]) && !empty($arItems["DELAY"])):?>
		<?foreach( $arItems["DELAY"] as $key=>$item ){?>
			$('.bigdata_recommended_products_items .wish_item.to[data-item=<?=$key?>]').hide();
			$('.bigdata_recommended_products_items .wish_item.in[data-item=<?=$key?>]').show();
			if ($('.bigdata_recommended_products_items .wish_item[data-item=<?=$key?>]').find(".value.added").length) {
				$('.bigdata_recommended_products_items .wish_item[data-item=<?=$key?>]').addClass("added");
				$('.bigdata_recommended_products_items .wish_item[data-item=<?=$key?>]').find(".value").hide();
				$('.bigdata_recommended_products_items .wish_item[data-item=<?=$key?>]').find(".value.added").css('display','block');
			}
		<?}?>
	<?endif;?>
