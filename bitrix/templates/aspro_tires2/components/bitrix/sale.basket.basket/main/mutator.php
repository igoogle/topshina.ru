...
foreach ($this->basketItems as $row)
{
	...
	else
	{
		...
		if($arElement && $arElement['IBLOCK_SECTION_ID'])
		{
			...
			{
				$img = CFile::ResizeImageGet($arSection['PICTURE'], array( "width" => 170, "height" => 170 ), BX_RESIZE_IMAGE_PROPORTIONAL,true );
				$rowData['IMAGE_URL'] = $img['src'];
			}
			else
				$rowData['IMAGE_URL'] = \Aspro\Functions\CAsproTires2::showNoImage($arElement["IBLOCK_ID"]);
		}
		else
			$rowData['IMAGE_URL'] = \Aspro\Functions\CAsproTires2::showNoImage("");
		/**/
	}
