<?
	...
	{
		usort($arResult["GRID"]["ROWS"], 'CTires2::cmpByID');
		$arImages = array();
		foreach($arResult["GRID"]["ROWS"] as $key=>$arItem)
		{
			...
			if(!$arItem['PREVIEW_PICTURE'] && !$arItem['DETAIL_PICTURE']){
				...
				if($arElement && $arElement['IBLOCK_SECTION_ID'])
				{
					...
					{
						$img = CFile::ResizeImageGet($arSection['PICTURE'], array( "width" => 70, "height" => 70 ), BX_RESIZE_IMAGE_PROPORTIONAL,true );
						$arResult["GRID"]["ROWS"][$key]['PREVIEW_PICTURE']['SRC'] = $img['src'];
					}
					else
						$arResult["GRID"]["ROWS"][$key]['PREVIEW_PICTURE']['SRC'] = \Aspro\Functions\CAsproTires2::showNoImage($arElement["IBLOCK_ID"]);
				}
				else{
					$arResult["GRID"]["ROWS"][$key]['PREVIEW_PICTURE']['SRC'] = \Aspro\Functions\CAsproTires2::showNoImage("");
				/**/
				}
			}
			...
			if(strpos($arItem["PRODUCT_XML_ID"], "#")!==false){
				...
				$arItem1 =CTires2Cache::CIBLockElement_GetList(array('CACHE' => array("MULTI"=>"N", "TAG" => CTires2Cache::GetIBlockCacheTag($arItem["IBLOCK_ID"]))), array("IBLOCK_ID" => $arItem["IBLOCK_ID"], "ACTIVE"=>"Y", "ACTIVE_DATE" => "Y", "XML_ID" => $arXmlID[0]), false, false, array("ID", "IBLOCK_ID"));
		...
		usort($arResult["GRID"]["HEADERS"], 'CTires2::cmpBySort');
