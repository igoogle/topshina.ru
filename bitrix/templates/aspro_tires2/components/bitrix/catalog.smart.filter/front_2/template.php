...
if($arResult["ITEMS"]){?>
	...
	$bShowTires = ($arParams['SHOW_FILTER_TIRES'] == 'Y');
	$bShowWheels = ($arParams['SHOW_FILTER_WHEELS'] == 'Y');
	$bShowAkb = ($arParams['SHOW_FILTER_AKB'] == 'Y');

	if (!$bShowTires && !CTires2::checkAjaxRequest()) {
		if ($bShowWheels) {
			$arParams["IBLOCK_ID"] = \Bitrix\Main\Config\Option::get(
				"aspro.tires2",
				"CATALOG_WHEELS_IBLOCK_ID",
				CTires2Cache::$arIBlocks[SITE_ID]["aspro_tires2_catalog"]["aspro_tires2_catalog_wheels"][0]
			);
			$arParams["FILTER_URL"] = $arTheme["CATALOG_WHEELS_PAGE_URL"]["VALUE"];
			$arParams["TYPE"] = "wheels";
		} elseif ($bShowAkb) {
			$arParams["IBLOCK_ID"] = \Bitrix\Main\Config\Option::get(
				"aspro.tires2",
				"CATALOG_AKB_IBLOCK_ID",
				CTires2Cache::$arIBlocks[SITE_ID]["aspro_tires2_catalog"]["aspro_tires2_catalog_accumulators"][0]
			);
			$arParams["FILTER_URL"] = $arTheme["CATALOG_AKB_PAGE_URL"]["VALUE"];
			$arParams["TYPE"] = "akb";
		}
	}
	...
	switch($arParams['IBLOCK_ID']){
		case $arShowTyreIndex[0]:
			$title = strip_tags($arParams['BLOCK_TITLE_TIRES']);
			$bTiresIBlockID = true;
			break;
		case $arShowTyreIndex[1]:
			$title = strip_tags($arParams['BLOCK_TITLE_WHEELS']);
			$bWheelsIBlockID = true;
			break;
		case $arShowTyreIndex[2]:
			$title = strip_tags($arParams['BLOCK_TITLE_AKB']);
			break;
		case $arShowTyreIndex[3]:
			$title = strip_tags($arParams['BLOCK_TITLE_TIRES_MOTO']);
			break;
		case $arShowTyreIndex[4]:
			$title = strip_tags($arParams['BLOCK_TITLE_TIRES_TRUCK']);
			break;
	}
	?>
	...
	<div class="front_filter_wrap type_2 <?=($arParams['TYPE'] ? $arParams['TYPE'] : '');?>" data-params='<?=str_replace('\'', '"', CUtil::PhpToJSObject($arParams, false))?>' data-template="<?=$this->__component->__template->__name?>">
		<div class="left_block pull-left">
			<div class="tabs_select">
				<?if($bShowTires):?>
					<div class="item_tab<?=($arParams['IBLOCK_ID'] == $arShowTyreIndex[0] || $arParams['IBLOCK_ID'] == $arShowTyreIndex[3] || $arParams['IBLOCK_ID'] == $arShowTyreIndex[4] ? ' active' : '');?>" data-iblock_id="<?=$arShowTyreIndex[0];?>" data-filter_url="<?=$arPropType['CATALOG_TIRES_PAGE_URL']['VALUE'];?>" data-type="tires">
						<span class="text"><?=\Aspro\Functions\CAsproTires2::showIconSvg('', SITE_TEMPLATE_PATH.'/images/svg/filter_tire.svg');?><span><?=($arParams['FILTER_TIRES_TITLE'] ? htmlspecialchars_decode($arParams['FILTER_TIRES_TITLE']) : Loc::getMessage('TIRES_TAB_TITLE'));?></span></span>
					</div>
				<?endif;?>
				<?if($bShowWheels):?>
					<div class="item_tab<?=($arParams['IBLOCK_ID'] == $arShowTyreIndex[1] && CTires2::checkAjaxRequest() ? ' active' : '');?>" data-iblock_id="<?=$arShowTyreIndex[1];?>" data-filter_url="<?=$arPropType['CATALOG_DISK_PAGE_URL']['VALUE'];?>" data-type="wheels">
						<span class="text"><?=\Aspro\Functions\CAsproTires2::showIconSvg('', SITE_TEMPLATE_PATH.'/images/svg/filter_disk.svg');?><span><?=($arParams['FILTER_WHEELS_TITLE'] ? htmlspecialchars_decode($arParams['FILTER_WHEELS_TITLE']) : Loc::getMessage('WHEELS_TAB_TITLE'));?></span></span>
					</div>
				<?endif;?>
				<?if($bShowAkb):?>
					<div class="item_tab<?=($arParams['IBLOCK_ID'] == $arShowTyreIndex[2] && CTires2::checkAjaxRequest() ? ' active' : '');?>" data-iblock_id="<?=$arShowTyreIndex[2];?>" data-filter_url="<?=$arPropType['CATALOG_AKB_PAGE_URL']['VALUE'];?>" data-type="akb">
						<span class="text"><?=\Aspro\Functions\CAsproTires2::showIconSvg('', SITE_TEMPLATE_PATH.'/images/svg/filter_battery.svg');?><span><?=($arParams['FILTER_AKB_TITLE'] ? htmlspecialchars_decode($arParams['FILTER_AKB_TITLE']) : Loc::getMessage('AKB_TAB_TITLE'));?></span></span>
					</div>
				<?endif;?>
			</div>
		</div>
		<div class="right_block <?=((!$bShowTires && !CTires2::checkAjaxRequest()) ? 'no-tires' : '');?>">
			<div class="title_block">
				...
				<?endif;?>
				<div class="title<?=(!$arRegion ? ' not_region' : '');?><?=($countTyres > 1 && $bShowDropdownTyresTitles && strpos($arParams['TYPE'], 'tires') !== false ? ' with_dropdown' : '');?>">
				...
				<?if($arRegion):?>
					<span class="region_name"><?=Loc::getMessage('IN');?>#REGION_NAME_DECLINE_PP#</span>
				<?endif;?>
				<?if($arParams['IBLOCK_ID'] == $arShowTyreIndex[0] || $arParams['IBLOCK_ID'] == $arShowTyreIndex[1] || $arParams['IBLOCK_ID'] == $arShowTyreIndex[2]):?>
					<div class="links">
				...
				<div class="bx_filter_section">
					...
					<input type="hidden" <?=($arPropType["PODBOR_ELEMENTS_URL"]["VALUE"] ? 'name="'.$arPropType["PODBOR_ELEMENTS_URL"]["VALUE"].'"' : '');?> id="<?=$arPropType["PODBOR_ELEMENTS_URL"]["VALUE"];?>" value="Y">
													...
													<div class="bx_filter_parameters_box_container_block">
														<div class="bx_filter_input_container form-control bg">
															<input
																...
																onkeyup="smartFilter<?=$arParams["AJAX_FILTER_FLAG"]?>.keyup(this)"
															/>
														</div>
													</div>
													<div class="bx_filter_parameters_box_container_block">
						...
						foreach($arResult["ITEMS"] as $key=>$arItem)
						{
							...
							<div class="bx_filter_parameters_box type_<?=$arItem["DISPLAY_TYPE"];?> <?=$class;?>" data-tyreindex="<?=$typeTI;?>" data-expanded="<?=($arItem["DISPLAY_EXPANDED"] ? $arItem["DISPLAY_EXPANDED"] : "N");?>" data-rnf="<?=($arPropType["PROP_TYRE_RNF"]["VALUE"] == $arItem["CODE"] ? "Y" : "N");?>" data-prop_code="<?=strtolower($arPropType['PROP_TYRE_'.$arItem['CODE']]['VALUE']);?>" data-prop_code_wheels="<?=strtolower($arPropType['PROP_DISK_'.$arItem['CODE']]['VALUE']);?>" data-prop_code_akb="<?=strtolower($arPropType['PROP_AKB_'.$arItem['CODE']]['VALUE']);?>" data-property_id="<?=$arItem["ID"]?>">
								<span class="bx_filter_container_modef"></span>
									...
									switch ($arItem["DISPLAY_TYPE"]){
												...
												<?if($ar["VALUE"]){?>
													...
													<div class="pict">
														...
														if ($ar["DISABLED"])
															$class.= " disabled";
														?>
														<label for="<?=$ar["CONTROL_ID"]?>" data-role="label_<?=$ar["CONTROL_ID"]?>" class="bx_filter_param_label nab dib<?=$class?>" onclick="smartFilter<?=$arParams["AJAX_FILTER_FLAG"]?>.keyup(BX('<?=CUtil::JSEscape($ar["CONTROL_ID"])?>')); BX.toggleClass(this, 'active');">
											...
											<?foreach ($arItem["VALUES"] as $val => $ar):?>
												<?if($ar["VALUE"]){?>
													...
													if ($ar["DISABLED"])
														$class.= " disabled";
													?>
													<label for="<?=$ar["CONTROL_ID"]?>" data-role="label_<?=$ar["CONTROL_ID"]?>" class="bx_filter_param_label<?=$class?> pal nab" onclick="smartFilter<?=$arParams["AJAX_FILTER_FLAG"]?>.keyup(BX('<?=CUtil::JSEscape($ar["CONTROL_ID"])?>')); BX.toggleClass(this, 'active');">
														<?/*<span class="bx_filter_param_btn bx_color_sl" title="<?=$ar["VALUE"]?>">*/?>
															<?if (isset($ar["FILE"]) && !empty($ar["FILE"]["SRC"])):?>
										...
										case "P"://DROPDOWN
											...
											<div class="bx_filter_select_container">
												<div class="bx_filter_select_block" onclick="smartFilter<?=$arParams["AJAX_FILTER_FLAG"]?>.showDropDownPopup(this, '<?=CUtil::JSEscape($key)?>')">
													...
													<div class="bx_filter_select_popup" data-role="dropdownContent" style="display: none;">
														<ul>
															<li>
																<label for="<?="all_".$arCur["CONTROL_ID"]?>" class="bx_filter_param_label" data-role="label_all_<?=$arCur["CONTROL_ID"]?>" onclick="smartFilter<?=$arParams["AJAX_FILTER_FLAG"]?>.selectDropDownItem(this, '<?=CUtil::JSEscape("all_".$arCur["CONTROL_ID"])?>')">
																	<? echo Loc::getMessage("CT_BCSF_FILTER_ALL"); ?>
																</label>
															</li>
														<?
														...
														?>
															<li>
																<label for="<?=$ar["CONTROL_ID"]?>" class="bx_filter_param_label<?=$class?> <?=($ar["EXT_CLASS"] ? $ar["EXT_CLASS"] : "");?>" data-role="label_<?=$ar["CONTROL_ID"]?>" onclick="smartFilter<?=$arParams["AJAX_FILTER_FLAG"]?>.selectDropDownItem(this, '<?=CUtil::JSEscape($ar["CONTROL_ID"])?>')"><?=$ar["VALUE"]?></label>
															</li>
														<?endforeach?>
														</ul>
													</div>
													...
													<div class="bx_filter_select_popup" data-role="dropdownContent" style="display: none">
														<ul>
															<li style="border-bottom: 1px solid #e5e5e5;padding-bottom: 5px;margin-bottom: 5px;">
																<label for="<?="all_".$arCur["CONTROL_ID"]?>" class="bx_filter_param_label" data-role="label_<?=$arCur["CONTROL_ID"]?>" onclick="smartFilter<?=$arParams["AJAX_FILTER_FLAG"]?>.selectDropDownItem(this, '<?=CUtil::JSEscape("all_".$arCur["CONTROL_ID"])?>')">
																	<? echo Loc::getMessage("CT_BCSF_FILTER_ALL"); ?>
																</label>
															</li>
														<?
														...
														?>
															<li>
																<label for="<?=$ar["CONTROL_ID"]?>" data-role="label_<?=$ar["CONTROL_ID"]?>" class="bx_filter_param_label<?=$class?>" onclick="smartFilter<?=$arParams["AJAX_FILTER_FLAG"]?>.selectDropDownItem(this, '<?=CUtil::JSEscape($ar["CONTROL_ID"])?>')">
																	<?if (isset($ar["FILE"]) && !empty($ar["FILE"]["SRC"])):?>
																		<span class="bx_filter_btn_color_icon" title="<?=$ar["VALUE"]?>" style="background-image:url('<?=$ar["FILE"]["SRC"]?>');"></span>
																	<?endif?>
																	<span class="bx_filter_param_text">
										...
										case "K"://RADIO_BUTTONS
											...
											<div class="filter label_block radio">
												<input
													...
													id="<? echo "all_".$arCur["CONTROL_ID"] ?>"
													onclick="smartFilter<?=$arParams["AJAX_FILTER_FLAG"]?>.click(this)"
												/>
										...
										default://CHECKBOXES
											...
											<?if(isset($arItem["EXT_RNF"])):?>
												<?foreach($arItem["EXT_RNF"][0]["VALUES"] as $val => $ar):?>
													<input
														type="checkbox"
														value="<? echo $ar["HTML_VALUE"] ?>"
														name="<? echo $ar["CONTROL_NAME"] ?>"
							...
							<?if($isFilter){?>
								<div class="bx_filter_block pull-left">
									<div class="bx_filter_parameters_box_container">
										...
										<input class="bx_filter_search_button btn btn-default" onclick="return sendQueryTires<?=$arParams["AJAX_FILTER_FLAG"]?>(this)" type="submit" name="set_filter" data-href="" value="<?=Loc::getMessage("CT_BCSF_SET_FILTER")?>" />
	...
	<script>
		var sendQueryTires<?=$arParams["AJAX_FILTER_FLAG"]?> = function(e){
			var form = $(e).closest("form");
			smartFilter<?=$arParams["AJAX_FILTER_FLAG"]?>.click(form.find("input").first()[0], true, form.find('[name=set_filter]')[0])
			return false;
		}
