$(document).on('click', '.front_filter_wrap .title_block .dropdown .item, .front_filter_wrap.type_2 .left_block .tabs_select .item_tab:not(.active)', function(){
	console.log(22)
	var _this = $(this),
...
$(document).ready(function(){
	$('.front_filter_wrap .title_block').sliceHeight();
	...
	$(document).on('click', '.bx_filter_search_reset_main', function(){
		...
		$(this).closest('.bx_filter').find('.min-price, .max-price').each(function(){
			var _this = $(this);
			
			_this.val('');
			_this.closest('.bx_filter_block').find('.bx_ui_slider_handle.left').css('left', 0);
			_this.closest('.bx_filter_block').find('.bx_ui_slider_handle.right').css('right', 0);
			_this.closest('.bx_filter_block').find('.bx_ui_slider_pricebar_VD').css({'right': 0, 'left': 0});
			_this.closest('.bx_filter_block').find('.bx_ui_slider_pricebar_V').css({'right': 0, 'left': 0});
		});
		...
		if($(this).closest('.front_filter_wrap.akb').length){
			$('.block_all_results .akb_result').html('');
		}
		
		if($('.block_all_results .tyres_result').html() == '' && $('.block_all_results .wheels_result').html() == '' && $('.block_all_results .akb_result').html() == ''){
			$('#filter_result').removeClass('showen');
		}
	});
	...
	$(document).on('click', '.front_filter_wrap .title_block .link_item', function(){
		...
		if(!_this.hasClass('active')){
			...
			if(dataFilter == 'params'){
				...
				//_this.closest('.front_filter_wrap').find('.bx_filter_button_box').css('margin-bottom', '27px');
			}
				...
				//_this.closest('.front_filter_wrap').find('.bx_filter_button_box').css('margin-bottom', '32px');
			}
