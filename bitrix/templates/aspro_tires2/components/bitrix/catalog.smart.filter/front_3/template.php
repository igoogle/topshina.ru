...
if($arResult["ITEMS"]){?>
	...
	$bShowMTires = ($arParams['SHOW_FILTER_MOTO_TIRES'] == 'Y');
	$bShowTrTires = ($arParams['SHOW_FILTER_TRUCK_TIRES'] == 'Y');
	...
	if (!$bShowTires && !CTires2::checkAjaxRequest()) {
		if ($bShowWheels) {
			...
			$arParams["FILTER_URL"] = $arTheme["CATALOG_MOTO_TIRES_PAGE_URL"]["VALUE"];
			...
			$arParams["FILTER_URL"] = $arTheme["CATALOG_TRUCK_TIRES_PAGE_URL"]["VALUE"];
	...
	$bAjax = ((isset($_SERVER["HTTP_X_REQUESTED_WITH"]) && strtolower($_SERVER['HTTP_X_REQUESTED_WITH']) == "xmlhttprequest") && (isset($_POST["IS_AJAX"]) && $_POST["IS_AJAX"] == "Y"));
	...
	<div class="all_props_wrappers">
		<div class="tab_slider_wrapp filter specials best_block clearfix">
			<div class="top_blocks clearfix">
				...
				<?endif;?>
				<?if($arParams['SHOW_FILTER_TIRES'] == 'Y' || $arParams['SHOW_FILTER_MOTOT_TIRES'] == 'Y' || $arParams['SHOW_FILTER_TRUCK_TIRES'] == 'Y' || $arParams['SHOW_FILTER_AKB'] == 'Y' || $arParams['SHOW_FILTER_WHEELS'] == 'Y'):?>
					<ul class="tabs filter" data-params='<?=str_replace('\'', '"', CUtil::PhpToJSObject($arParams, false))?>' data-template="<?=$this->__component->__template->__name?>">
						<?if($bShowTires):?>
							<li data-iblock_id="<?=$arShowTyreIndex[0];?>" data-filter_url="<?=$arPropType['CATALOG_TIRES_PAGE_URL']['VALUE'];?>" data-type="tires" class="cur"><span><?=strip_tags($arParams['FILTER_TIRES_TITLE']);?></span></li>
						<?endif;?>
						<?if($bShowWheels):?>
							<li data-iblock_id="<?=$arShowTyreIndex[1];?>" data-filter_url="<?=$arPropType['CATALOG_DISK_PAGE_URL']['VALUE'];?>" data-type="wheels" <?=($arParams['IBLOCK_ID'] == $arShowTyreIndex[1] && CTires2::checkAjaxRequest() ? ' class="cur"' : '');?>><span><?=strip_tags($arParams['FILTER_WHEELS_TITLE']);?></span></li>
						<?endif;?>
						<?if($bShowAkb):?>
							<li data-iblock_id="<?=$arShowTyreIndex[2];?>" data-filter_url="<?=$arPropType['CATALOG_AKB_PAGE_URL']['VALUE'];?>" data-type="akb" <?=($arParams['IBLOCK_ID'] == $arShowTyreIndex[2] && CTires2::checkAjaxRequest() ? ' class="cur"' : '');?>><span><?=strip_tags($arParams['FILTER_AKB_TITLE']);?></span></li>
						<?endif;?>
						<?if($bShowMTires):?>
							<li data-iblock_id="<?=$arShowTyreIndex[3];?>" data-filter_url="<?=$arPropType['CATALOG_MOTO_TIRES_PAGE_URL']['VALUE'];?>" data-type="moto tires" <?=($arParams['IBLOCK_ID'] == $arShowTyreIndex[3] && CTires2::checkAjaxRequest() ? ' class="cur"' : '');?>><span><?=strip_tags($arParams['FILTER_MOTO_TIRES_TITLE']);?></span></li>
						<?endif;?>
						<?if($bShowTrTires):?>
							<li data-iblock_id="<?=$arShowTyreIndex[4];?>" data-filter_url="<?=$arPropType['CATALOG_TRUCK_TIRES_PAGE_URL']['VALUE'];?>" data-type="truck tires" <?=($arParams['IBLOCK_ID'] == $arShowTyreIndex[4] && CTires2::checkAjaxRequest() ? ' class="cur"' : '');?>><span><?=strip_tags($arParams['FILTER_TRUCK_TIRES_TITLE']);?></span></li>
						<?endif;?>
						<li class="stretch"></li>
					</ul>
				<?endif;?>
		...
		<div class="front_filter_wrap_inner" data-params='<?=str_replace('\'', '"', CUtil::PhpToJSObject($arParams, false))?>' data-template="<?=$this->__component->__template->__name?>">

			<?if($bAjax):?>
				<?$APPLICATION->RestartBuffer();?>
			<?endif;?>

			<div class="front_filter_wrap catalog_filter <?=($arParams['TYPE'] ? $arParams['TYPE'] : '');?> <?=((!$bShowTires && !CTires2::checkAjaxRequest()) ? 'no-tires' : '');?>">
						...
						<form name="<?echo $arResult["FILTER_NAME"]."_form"?>" action="<?=$arResult["FORM_ACTION"]?>" method="get" class="smartfilter">
							...
							<div class="tyres_params active">
								...
								<?if($arResult["TOP_PROPS"]):?>
									<?=showFilterProps($arResult["TOP_PROPS"], $arPropType, $numVisiblePropValues, $arParams);?>
								<?endif;?>

								<?//prices?>
								<?if($arResult["OTHER_PROPS"]):?>
									<div class="other_props">
										<?=showFilterProps($arResult["OTHER_PROPS"], $arPropType, $numVisiblePropValues, $arParams);?>
									</div>
								<?endif;?>
							</div>
