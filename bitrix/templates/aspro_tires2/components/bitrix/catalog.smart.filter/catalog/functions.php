...
function showFilterProps($arItems, $arPropType, $numVisiblePropValues, $arParams){
	$i == 0;
				...
				switch ($arItem["DISPLAY_TYPE"]){
						...
						<div class="bx_filter_select_container">
							<div class="bx_filter_select_block" onclick="smartFilter.showDropDownPopup(this, '<?=CUtil::JSEscape($key)?>')">
								...
								<div class="bx_filter_select_popup" data-role="dropdownContent" style="display: none;">
									<ul>
										<li>
											<label for="<?="all_".$arCur["CONTROL_ID"]?>" class="bx_filter_param_label" data-role="label_all_<?=$arCur["CONTROL_ID"]?>" onclick="smartFilter.selectDropDownItem(this, '<?=CUtil::JSEscape("all_".$arCur["CONTROL_ID"])?>')">
												<? echo Loc::getMessage("CT_BCSF_FILTER_ALL"); ?>
											</label>
										</li>
									<?
					...
					default://CHECKBOXES
						...
						<?if(isset($arItem["EXT_RNF"])):?>
							<?=showFilterProps($arItem["EXT_RNF"], $arPropType, $numVisiblePropValues, $arParams);?>
						<?endif;?>
				<?}?>
				</div>
				<div class="clb"></div>
				...
				else
				{
					...
					else
					{
						foreach($arItem["VALUES"] as $key2 => $arValue)
						{
							...
							if(
								$arItem["CODE"] == $arPropType["PROP_TYRE_RNF"]["VALUE"] 
								&& $arPropType["PROP_TYRE_RNF_ONE"]["VALUE"] != "N"
							) {
								if($arValue["VALUE"] == $arPropType["PROP_TYRE_RNF_Y"]["VALUE"]) {
									$arItems[$key]["VALUES"][$key2]["EXT_CLASS"] = "icon-rnf";
								} else {
									unset($arItems[$key]["VALUES"][$key2]);
								}
							}
