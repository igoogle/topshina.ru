	...
	<?elseif($arResult["TYPE_T"] == 'akb'):?>
		<div class="filter-b inb">
			...
			<div class="bx_filter_block limited_block">
				<div class="bx_filter_parameters_box_container">
					<?
					$PropAkbCode = \Bitrix\Main\Config\Option::get('aspro.tires2', 'PROP_AKB_POLARNOST', 'POLARNOST', SITE_ID);
					$AkbIBlock = \Bitrix\Main\Config\Option::get('aspro.tires2', 'CATALOG_AKB_IBLOCK_ID', CTires2Cache::$arIBlocks[SITE_ID]["aspro_tires2_catalog"]["aspro_tires2_catalog_accumulators"][0], SITE_ID);

					$resultProp = CIBlockPropertyEnum::GetList(array('SORT' => 'ASC'), array('CODE' => $PropAkbCode, 'IBLOCK_ID' => $AkbIBlock));
					while($prop = $resultProp->Fetch()){
						$PropAkbList[] = $prop;
					}
					?>
					<?foreach($arResult["TYPE"] as $name => $arData):?>
						<?if(!empty($arData)):?>
							<?
							$arData['polarity_lang_id'] = $arData['polarity_lang'];
							if($PropAkbList) {
								foreach ($PropAkbList as $prop) {
									if($arData['polarity_lang'] == $prop['VALUE']) {
										$arData['polarity_lang_id'] = $prop['ID'];
									}
								}
							}
