<?if(!defined("B_PROLOG_INCLUDED") || B_PROLOG_INCLUDED!==true)die();?>
	...
	<div class="form-wr">
		<form method="post" id="one_click_buy_form" action="<?=$arResult['SCRIPT_PATH']?>/script.php">
			...
			<?endforeach;?>
			<?if(\Bitrix\Main\Config\Option::get('aspro.tires2', 'ONE_CLICK_BUY_CAPTCHA', 'N') == 'Y'):?>
			...
			<?if($arParams["SHOW_LICENCE"] == "Y"):?>
				<div class="form">
					<div class="licence_block filter label_block">
						<input type="checkbox" id="licenses_popup_OCB" <?=(COption::GetOptionString("aspro.tires2", "LICENCE_CHECKED", "N") == "Y" ? "checked" : "");?> name="licenses_popup_OCB" required value="Y">
			...
			<input type="hidden" name="STORE" value="<?=$arParams['STORE']?>" />
			<input type="hidden" name="PRICES" value="<?=$arParams['PRICES']?>" />
			<?=bitrix_sessid_post()?>
		</form>
...
<?if(!$arParams['INLINE_FORM']):?>
	if(arTires2Options['THEME']['PHONE_MASK']){
		$('#one_click_buy_id_PHONE').inputmask( "mask", { "mask": arTires2Options['THEME']['PHONE_MASK'], "showMaskOnFocus": true } );
	}
<?endif;?>
</script>