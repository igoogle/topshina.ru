...
?>
<?$bStoreBuy = (\Bitrix\Main\Config\Option::get('aspro.tires2', 'STORE_TYPE_CATALOG_DETAIL', 'ext') == 'ext');
if($bStoreBuy)
	$arItems2=CTires2::getBasketItemsWithProp();?>
...
else
{
	...
	{
		$sectionParent = CTires2Cache::CIBlockSection_GetList(array('CACHE' => array("MULTI" =>"N", "TAG" => CTires2Cache::GetIBlockCacheTag($arParams["IBLOCK_ID"]))), array('GLOBAL_ACTIVE' => 'Y', "ID" => $arSection["IBLOCK_SECTION_ID"], "IBLOCK_ID" => $arParams["IBLOCK_ID"]), false, array("ID", "IBLOCK_ID", "NAME", "UF_ELEMENT_DETAIL"));
...
<?$arAllValues=$arSimilar=$arAccessories=$arSimilarParams=array();
$arShowTabs = array("element_1", "element_2");

/*similar params*/
$arFilterSimilarProps = array();
if($bShowSimilarByParamsItems && $arElement)
{
	if($arTmpSelectProps)
	{
		foreach($arElement as $key => $value)
		{
			...
			{
				if(strpos($key, "VALUE_ID") === false && strpos($key, "ENUM_ID") === false)
					$arFilterSimilarProps[str_replace(array("PROPERTY_", "_VALUE"), "", $key)]["VALUE"] = $value;
			}
		}
		if($arFilterSimilarProps)
		{
			$arTmpFilterSimilarProps = array();
			foreach($arFilterSimilarProps as $key => $arPropInfo)
			{
				$arProp = CIBlockProperty::GetList(array("id" => "asc"), array("IBLOCK_ID" => $arParams["IBLOCK_ID"], "CODE" => $key))->Fetch();
				$arFilterSimilarProps[$arProp["CODE"]]["TYPE"] = $arProp["PROPERTY_TYPE"];

				if($arProp["PROPERTY_TYPE"] == "L")
					$arTmpFilterSimilarProps["PROPERTY_".$key."_VALUE"] = $arFilterSimilarProps[$arProp["CODE"]]["VALUE"];
				else
					$arTmpFilterSimilarProps["PROPERTY_".$key] = $arFilterSimilarProps[$arProp["CODE"]]["VALUE"];
			}

			$arAllValues["SIMILAR_PARAMS"] = array(
				"LOGIC" => "AND",
				"!ID" => $arElement["ID"],
				$arTmpFilterSimilarProps
			);
		}
	}
}
...
<div class="catalog_detail detail<?=($isWideBlock == "Y" ? " fixed_wrapper" : "");?> element_4" itemscope itemtype="http://schema.org/Product">
	<?@include_once('page_blocks/'.$sViewElementTemplate.'.php');?>
</div>

<?CTires2::checkBreadcrumbsChain($arParams, $arSection, $arElement);?>
<div class="clearfix"></div>

<?
if(!in_array($sViewElementTemplate, $arWidePage)):
/*similar goods*/
$arExpValues=CTires2Cache::CIBlockElement_GetProperty($arParams["IBLOCK_ID"], $ElementID, array("CACHE" => array("TAG" =>CTires2Cache::GetIBlockCacheTag($arParams["IBLOCK_ID"]))), array("CODE" => "EXPANDABLES"));
if($arExpValues){
	$arAllValues["EXPANDABLES"]=$arExpValues;
}
/*accessories goods*/
$arAccessories=CTires2Cache::CIBlockElement_GetProperty($arParams["IBLOCK_ID"], $ElementID, array("CACHE" => array("TAG" =>CTires2Cache::GetIBlockCacheTag($arParams["IBLOCK_ID"]))), array("CODE" => "ASSOCIATED"));
if($arAccessories){
