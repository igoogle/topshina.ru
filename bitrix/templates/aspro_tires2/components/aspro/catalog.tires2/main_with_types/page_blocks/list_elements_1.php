	...
	<div class="js_bottom_block">
		...
		<?if(!$iSectionsCount):?>
				...
				<div class="list_model_wrapper">
					<div class="list_model_Inner_wrapper">
						<div class="img_wrapper">
							...
							<?$arImages = $arImagesFormat = [];
							if ($arSection["PICTURE"]) {
								$arImages[] = $arSection["PICTURE"];
							}
							if ($arSection["UF_GALLERY"]) {
								$arImages = array_merge($arImages, $arSection["UF_GALLERY"]);
							}
							if ($arImages) {
								foreach ($arImages as $i => $pictureID) {
									$arImage = array_merge(
										CFile::GetFileArray($pictureID),
										array(
											"BIG" => array('src' => CFile::GetPath($pictureID)),
											"SMALL" => CFile::ResizeImageGet($pictureID, array("width" => 500, "height" => 500), BX_RESIZE_IMAGE_PROPORTIONAL_ALT, true, array()),
											"THUMB" => CFile::ResizeImageGet($pictureID, array("width" => 50, "height" => 50), BX_RESIZE_IMAGE_PROPORTIONAL_ALT, true, array())
										)
									);
									$arImagesFormat[$i] = $arImage;
								}
								unset($arImages);
							}
							?>
							<?$countThumb = count($arImagesFormat);?>
							<?if($arImagesFormat):?>
								<div class="big-img-wrapper has_<?=($countThumb > 1 ? 'more' : 'one');?>">
									<?foreach ($arImagesFormat as $i => $arImage):?>
										<div id="photo-<?=$i?>" class="img_inner <?=(!$i ? 'current' : '')?>">
											<a href="<?=$arImage["SRC"];?>" class="popup_link fancy" data-fancybox-group="item_slider" title="<?=($arImage["DESCRIPTION"] ? $arImage["DESCRIPTION"] : $arSection["NAME"]);?>">
								...
								<div class="catalog_detail">
									<div class="item_slider">
										<div class="wrapp_thumbs xzoom-thumbs">
											<?if($countThumb > 1):?>
												<div class="thumbs flexslider" data-plugin-options='{"animation": "slide", "selector": ".slides_block > li", "directionNav": true, "itemMargin":10, "itemWidth": 54, "controlsContainer": ".thumbs_navigation", "controlNav" :false, "animationLoop": true, "slideshow": false}' style="max-width:<?=ceil((($countThumb <= 4 ? $countThumb : 4) * 64) - 10)?>px;">
												...
												<div class="brand">
													...
													<a href="<?=$sectionRoot["SECTION_PAGE_URL"]?>" title="<?=$sectionRoot["NAME"]?>">
														<?if($picture):?>
															...
															<img src="<?=$arImage["SRC"];?>" alt="<?=$sectionRoot["NAME"];?>" title="<?=($arImage["DESCRIPTION"] ? $arImage["DESCRIPTION"] : $sectionRoot["NAME"]);?>" />
														<?else:?>
															<?=$sectionRoot["NAME"]?>
														<?endif;?>
													</a>
					...
					<?
					/*get tizers section*/
					if(is_array($arSection["UF_TIZERS"]) && $arSection["UF_TIZERS"] && \Bitrix\Main\Loader::includeModule('highloadblock')){
						$arTizersData = array();
			...
			<?endif;?>
				...
				<?endif;?>
				<?
				$_REQUEST['filter'] = ($_REQUEST['filter'] == 'all' ? '' : $_REQUEST['filter']);
				if ($arParams['IBLOCK_ID'] == $arTheme['CATALOG_WHEELS_IBLOCK_ID']['VALUE']) {
					$arTmpFilter = array('PROPERTY_POSADOCHNYY_DIAMETR_DISKA' => $_REQUEST['filter']);
				} else {
					$arTmpFilter = array('PROPERTY_POSADOCHNYY_DIAMETR' => $_REQUEST['filter']);
				}
				
				if(is_array($GLOBALS[$arParams["FILTER_NAME"]])){
					$GLOBALS[$arParams["FILTER_NAME"]] = array_merge($GLOBALS[$arParams["FILTER_NAME"]], $arTmpFilter);
				} else {
					$GLOBALS[$arParams["FILTER_NAME"]] = $arTmpFilter;
				}
				// print_r($GLOBALS[$arParams["FILTER_NAME"]]);
				$sort = ($arParams['PROP_TYRE_POSADOCHNYY_DIAMETR'] ? 'PROPERTY_POSADOCHNYY_DIAMETR' : 'PROPERTY_POSADOCHNYY_DIAMETR_DISKA');
				?>
				<?$APPLICATION->IncludeComponent(
		...
		<?else:?>
			<div class="section_block_wrapper">
									...
									<?if($arSection["PICTURE"]):?>
										...
										<div class="detailimage">
											<div class="img-partner">
												<img src="<?=$arImage["SRC"];?>" alt="<?=$arSection["NAME"];?>" title="<?=($arImage["DESCRIPTION"] ? $arImage["DESCRIPTION"] : $arSection["NAME"]);?>" class="img-responsive"/>
											</div>
										</div>
									<?endif;?>
				...
				if($request["display"] || array_key_exists("DISPLAY_SECTION_TEMPLATE", $_SESSION) || $arParams["DEFAULT_LIST_TEMPLATE"])
				{
				...
				<?endif;?>
					...
					<?$APPLICATION->IncludeComponent(
						...
						Array(
							...
							"STORES" => ($arRegion ? $arParams["STORES"] : ""),
							"PRICE_CODE" => $arParams["PRICE_CODE"],
							"CONVERT_CURRENCY" => $arParams["CONVERT_CURRENCY"],
							"CURRENCY_ID" => $arParams["CURRENCY_ID"],
							"HIDE_NOT_AVAILABLE" => $arParams["HIDE_NOT_AVAILABLE"],
