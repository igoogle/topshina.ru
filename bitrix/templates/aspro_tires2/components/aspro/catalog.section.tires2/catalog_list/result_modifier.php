	...
	foreach ($arResult['ITEMS'] as $key => $arItem)
	{
		...
		if(!$arItem['PREVIEW_PICTURE'] && !$arItem['DETAIL_PICTURE']){
			if($arSections[$arItem['IBLOCK_SECTION_ID']]['PICTURE']){
				...
				$arItem['NO_IMAGE_PATH'] = \Aspro\Functions\CAsproTires2::showNoImage($arItem["IBLOCK_ID"], ($arParams['SITE_ID'] ? $arParams['SITE_ID'] : ""));
			}
		}
		...
		foreach($arFormatProps as $codeProp)
		{
			...
			{
				$arItem["PROPERTIES"][$codeProp]["VALUE"] = round($arItem["PROPERTIES"][$codeProp]["VALUE"], 1);
			}
			if($arItem["DISPLAY_PROPERTIES"][$codeProp]["VALUE"])
			{
				$arItem["DISPLAY_PROPERTIES"][$codeProp]["DISPLAY_VALUE"] = round($arItem["DISPLAY_PROPERTIES"][$codeProp]["VALUE"], 1);
			}
		}
		/**/
		...
		if (!empty($arItem['DISPLAY_PROPERTIES']))
		{
			foreach ($arItem['DISPLAY_PROPERTIES'] as $propKey => $arDispProp)
			{
				...
				if ('F' == $arDispProp['PROPERTY_TYPE'] || $propKey=="CML2_ARTICLE" ||(in_array($propKey, \Aspro\Functions\CAsproTires2::$arHideProps)))
