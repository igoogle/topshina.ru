	        ...
	        listener:function(name, checked){
				...
				if(checked)
				{
					...
					else if(div_class == 'WITH_LEFT_BLOCK')
					{
						...
						$('.wrapper_inner.front > .right_block').removeClass('wide_Y').addClass('wide_N');
						$('.wrapper_inner.front > .left_block').removeClass('hidden');
				...
				else
				{
					...
					if(div_class == 'WITH_LEFT_BLOCK')
					{
						...
						$('.wrapper_inner.front > .right_block').removeClass('wide_N wide_').addClass('wide_Y');
						$('.wrapper_inner.front > .left_block').addClass('hidden');

						$(window).resize();
					}
				}

				if(div_class == 'CATALOG_TAB')
				{
				...
				//save option
				$.post(
					arTires2Options['SITE_DIR']+"ajax/options_save_mainpage.php",
					{
...
$(document).ready(function() {
	$('.style-switcher .presets').mCustomScrollbar({
		...
		callbacks:{
			onScroll: function(){
				...
				$.cookie('STYLE_SWITCHER_SCROLL_PRESET_POSITION', topPositionPresets, {path: arTires2Options['SITE_DIR']});
			}
		},
	...
	$('.style-switcher .left-block').mCustomScrollbar({
		...
		callbacks:{
			onScroll: function(){
				...
				$.cookie('STYLE_SWITCHER_SCROLL_LEFT_POSITION', topPositionLeftBlock, {path: arTires2Options['SITE_DIR']});
			}
		},
	...
	$('.style-switcher .right-block').mCustomScrollbar({
		...
		callbacks:{
			onScroll: function(){
				...
				$.cookie('STYLE_SWITCHER_SCROLL_RIGHT_POSITION', topPositionRightBlock, {path: arTires2Options['SITE_DIR']});
			}
		},
	...
	$('.style-switcher .can_save .save_btn').on('click', function(){
		...
		$.ajax({
			type:"POST",
			url:arTires2Options['SITE_DIR']+"ajax/options_save.php",
			data:{'SAVE_OPTIONS':'Y'},
			dataType:"json",
	...
	$(document).on('click', '.style-switcher .presets .preset-block', function(){
		...
		if(typeof arTires2Options.PRESETS === 'object' && typeof arTires2Options.PRESETS[i] === 'object'){
			var options = {
				'backurl': arTires2Options['SITE_DIR']
			};
			var order = [];
			...
			if(arTires2Options.PRESETS[i]['OPTIONS'] && typeof arTires2Options.PRESETS[i]['OPTIONS'] === 'object'){
				for(j in arTires2Options.PRESETS[i]['OPTIONS']){
					var val = arTires2Options.PRESETS[i]['OPTIONS'][j];
			...
			function _sendOptions(){
				$.ajax({
					...
					success: function(){
						...
						location.href = arTires2Options['SITE_DIR'];
					}
				});
			}
			...
			function _sendOrder(){
				if(order.length){
					var sort = order.pop();
					$.ajax({
						url: arTires2Options['SITE_DIR'] + 'ajax/options_save_mainpage.php',
						type: 'POST',
						data: sort,
						success: function(){
							_sendOrder();
