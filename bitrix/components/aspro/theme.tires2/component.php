...
$arFrontParametrs = CTires2::GetFrontParametrsValues(SITE_ID);

foreach(CTires2::$arParametrsList as $blockCode => $arBlock)
{
					...
					if($arResult[$optionCode]['SUB_PARAMS'][$key])
					{
						...
						foreach($arResult[$optionCode]['SUB_PARAMS'][$key] as $key2 => $arSubOptions)
						{
							...
							if(isset($arSubOptions['TEMPLATE']) && $arSubOptions['TEMPLATE'])
							{
								...
								// print_r($arResult['TEMPLATE_PARAMS'][$key]);
...
// $APPLICATION->AddHeadString(CTires2::GetBannerStyle($arResult['BANNER_WIDTH']['VALUE'], $arResult['TOP_MENU']['VALUE']), true);
...
$file = \Bitrix\Main\Application::getDocumentRoot().'/bitrix/components/aspro/theme.tires2/css/user_font_'.SITE_ID.'.css';
