...
if (!empty($arCompare) && is_array($arCompare))
{
	...
	while($obElement = $rsElements->GetNextElement())
	{
		...
		$arOffer = false;
		if(!$arItem["PREVIEW_PICTURE"] && !$arItem["DETAIL_PICTURE"] && \Aspro\Functions\CAsproTires2::getCatalogIBlocks($arItem["IBLOCK_ID"]))
		{
			$arItem["SECTION"] = CTires2Cache::CIBlockSection_GetList(array('CACHE' => array("MULTI" =>"N", "TAG" => CTires2Cache::GetIBlockCacheTag($arItem["IBLOCK_ID"]))), array('GLOBAL_ACTIVE' => 'Y', "ID" => $arItem["IBLOCK_SECTION_ID"], "IBLOCK_ID" => $arItem["IBLOCK_ID"]), false, array("ID", "IBLOCK_ID", "NAME","DETAIL_PICTURE", "PICTURE"));
		}
	...
	/*get iblocks*/
	$rsAllIBlocks = CIBlock::GetList(array("SORT" => "ASC", "ID" => "ASC"), array("ID" => $arTmpIBlocks));
	while($arAllIBlock = $rsAllIBlocks->Fetch())
		$arResult["IBLOCKS"][$arAllIBlock["ID"]] = $arAllIBlock["NAME"];
	/**/
