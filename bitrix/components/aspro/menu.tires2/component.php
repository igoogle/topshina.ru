...
$curDir = $Dir = $APPLICATION->GetCurDir();
...
while($Dir <> '')
{
	...
	$Dir = rtrim($Dir, "/");
	$menu_file_name = $io->CombinePath($_SERVER["DOCUMENT_ROOT"], $Dir, ".".$arParams["CHILD_MENU_TYPE"].".menu_ext.php");
	if($io->FileExists($menu_file_name))
	{
		$arParams["CACHE_PATH_EXT"] = $Dir."/";
		$bFounded = true;
		break;
	}
