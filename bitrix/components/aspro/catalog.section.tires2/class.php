...
class CatalogSectionComponent extends ElementList
{
	...
	public function onPrepareComponentParams($params)
	{
		...
		if(!$params['PROPERTY_CODE'])
			$params['PROPERTY_CODE'][] = 'HIT';

		/**
		 * ASPRO_COMMENT
		 * Set custom iblockID for multiply.
		 */
		if($params['IBLOCK_IDS'])
		{
			foreach($params['IBLOCK_IDS'] as $id)
			{
				$this->storage['IBLOCK_PARAMS'] += $this->getSingleIblockParams(array_merge($params, array('IBLOCK_ID' => $id)));
			}
			$params['IBLOCK_ID'] = reset($params['IBLOCK_IDS']);
		}
		/**/
	...
	protected function initSectionResult()
	{
		 ...
		 * Set custom iblockID for multiply.
		 */
		if($this->arParams['IBLOCK_IDS'])
			$filterFields['IBLOCK_ID'] = $this->arParams['IBLOCK_IDS'];
		/**/
	...
	protected function initCatalogInfo()
	{
		...
		{
			$catalogs = array();
			foreach($this->arParams['IBLOCK_IDS'] as $id)
			{
				$catalog = \CCatalogSku::GetInfoByIBlock($id);
	...
	protected function modifyDisplayProperties($iblock, &$iblockElements)
	{
		...
		if($this->arParams['IBLOCK_IDS']) //for multiply
		{
			//$iblock = $this->arParams['IBLOCK_IDS'];
			foreach($this->arParams['IBLOCK_IDS'] as $iblock)
			{
				parent::modifyDisplayProperties($iblock, $iblockElements);
			}
			return;
		}
		/**/
	...
	protected function getElementList($iblockId, $products)
	{
		...
		if($this->arParams['IBLOCK_IDS']) //for multiply
		{
			$iblockId = $this->arParams['IBLOCK_IDS'];
			$this->filterFields['IBLOCK_ACTIVE'] = 'Y';
		}
		/**/
	...
	/**
	 * ASPRO_COMMENT
	 * Check for correct iblocks for multiply.
	 */
	protected function checkIblock()
	{
		...
		if($bHasError && $this->arParams['IBLOCK_IDS'])
		{
			$iblocks = array();
			...
			{
				$iblocks[$iblock['IBLOCK_ID']] = $iblock['IBLOCK_ID'];
			}
			if($iblocks)
			{
				$this->arParams['IBLOCK_ID'] = reset($iblocks);
				$this->iblockProducts = array($this->arParams['IBLOCK_ID'] => '');
	...
	protected function processOffers()
	{
		if ($this->useCatalog && !empty($this->iblockProducts))
		{
			...
			if($this->productWithOffers)
			{
				foreach($this->productWithOffers as $key => $arItems)
				{
					foreach($arItems as $key2 => $value)
					{
						$arTmpOffers[$key2] = $key;
					}
				}
				foreach(array_keys($this->productWithOffers) as $iblock)
				{
					if(!array_key_exists($iblock, $this->storage['IBLOCK_PARAMS']))
						$this->storage['IBLOCK_PARAMS'][$iblock] = $this->storage['IBLOCK_PARAMS'][$this->arParams['IBLOCK_ID']];
