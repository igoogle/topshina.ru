...
//						unset($_SESSION["CAPTCHA_CODE"][$sid]);
	...
	if ($FORUM_TOPIC_ID > 0):
		...
		if (!is_array($arTopic) || $arTopic["FORUM_ID"] != $arParams["FORUM_ID"]):
			...
			{
				$oUserTypeEntity    = new CUserTypeEntity();
				$oUserTypeEntity->Update($iUserFieldId, array('UF_FORUM_MESSAGE_CNT' => 0)); 
			}
	...
	elseif (($arTopic = CForumTopic::GetList(array(), array("XML_ID" => "IBLOCK_".$arParams["ELEMENT_ID"]))->fetch()) && $arTopic):
		...
		{
			$oUserTypeEntity    = new CUserTypeEntity();
			$oUserTypeEntity->Update($iUserFieldId, array('UF_FORUM_TOPIC_ID' => $arTopic["ID"])); 
		}
		...
		{
			$oUserTypeEntity    = new CUserTypeEntity();
			$oUserTypeEntity->Update($iUserFieldId, array('UF_FORUM_MESSAGE_CNT' => $arTopic["POSTS"])); 
		}
