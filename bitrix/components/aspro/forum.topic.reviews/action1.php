...
//						unset($_SESSION["CAPTCHA_CODE"][$sid]);
	...
	$FORUM_TOPIC_ID = intVal($arResult["ELEMENT"]["UF_FORUM_TOPIC_ID"]);
	$FORUM_MESSAGE_CNT = intVal($arResult["ELEMENT"]["UF_FORUM_MESSAGE_CNT"]);
	
	if ($FORUM_TOPIC_ID <= 0):
	
		$prop_topic_id=0;
		$rsData = CUserTypeEntity::GetList(array(), array("ENTITY_ID" => "IBLOCK_".$PRODUCT_IBLOCK_ID."_SECTION", "XML_ID"=>"UF_FORUM_TOPIC_ID"));
		while($arRes = $rsData->Fetch()){$prop_topic_id = $arRes["ID"];}
		if (!$prop_topic_id) {$needProperty[] = "UF_FORUM_TOPIC_ID";}
	
		/*$db_res = CIBlockSection::GetList( array(), array("ID" => $arResult["ELEMENT"]["ID"]), false, array( "IBLOCK_ID", "ID", "NAME", "CODE", "UF_FORUM_TOPIC_ID", ), false);
		if (!($db_res && $res = $db_res->Fetch()))
			$needProperty[] = "UF_FORUM_TOPIC_ID";*/
	...
	if ($FORUM_MESSAGE_CNT <= 0):
		...
		$prop_message_cnt=0;
		$rsData = CUserTypeEntity::GetList(array(), array("ENTITY_ID" => "IBLOCK_".$PRODUCT_IBLOCK_ID."_SECTION", "XML_ID"=>"UF_FORUM_MESSAGE_CNT"));
		while($arRes = $rsData->Fetch()){$prop_message_cnt = $arRes["ID"];}
		if (!$prop_message_cnt) {$needProperty[] = "UF_FORUM_MESSAGE_CNT";}
	
		/*$db_res = CIBlockSection::GetList( array(), array("ID" => $arResult["ELEMENT"]["ID"]), false, array( "IBLOCK_ID", "ID", "NAME", "CODE", "UF_FORUM_MESSAGE_CNT", ), false);
		if (!($db_res && $res = $db_res->Fetch()))
			$needProperty[] = "UF_FORUM_MESSAGE_CNT";*/
	...
	if (!empty($needProperty)):
		...
		{
			$sName = trim($nameProperty == "UF_FORUM_TOPIC_ID" ? GetMessage("F_FORUM_TOPIC_ID") : GetMessage("F_FORUM_MESSAGE_CNT"));
			$sName = (empty($sName) ? $nameProperty : $sName);
						
			$oUserTypeEntity    = new CUserTypeEntity();
			$aUserFields    = array('ENTITY_ID'         => 'IBLOCK_'.$PRODUCT_IBLOCK_ID.'_SECTION',
									'FIELD_NAME'        => $nameProperty,
									'USER_TYPE_ID'      => 'double',
									'XML_ID'            => $nameProperty,
									'SORT'              => 500,
									'MULTIPLE'          => 'N',
									...
									'EDIT_IN_LIST'      => '',
									'IS_SEARCHABLE'     => 'N',
									'EDIT_FORM_LABEL'   => array('ru' => $sName),
									'LIST_COLUMN_LABEL' => array('ru' => $sName),
									'LIST_FILTER_LABEL' => array('ru' => $sName));						 
	...
	if ($FORUM_TOPIC_ID > 0):
		...
		if (!$arTopic || !is_array($arTopic) || count($arTopic) <= 0 || $arTopic["FORUM_ID"] != $arParams["FORUM_ID"]):
			...
			$rsData = CUserTypeEntity::GetList(array(), array("ENTITY_ID" => "IBLOCK_".$PRODUCT_IBLOCK_ID."_SECTION", "XML_ID"=>"UF_FORUM_TOPIC_ID"));
			while($arRes = $rsData->Fetch()){$iUserFieldId = $arRes["ID"];}
			if ($iUserFieldId)
			{
				$oUserTypeEntity    = new CUserTypeEntity();
				$oUserTypeEntity->Update($iUserFieldId, array('UF_FORUM_TOPIC_ID' => 0)); 
			}
	...
	// 1.7 Update Iblock Property
		if ($TID > 0)
		{
			...
			$ob->Update('IBLOCK_'.$PRODUCT_IBLOCK_ID.'_SECTION', $arParams["ELEMENT_ID"], array('UF_FORUM_TOPIC_ID'  => intVal($TID)));
		}
		else
		{
			if ($TOPIC_ID > 0)
