...
<?if(\Bitrix\Main\Loader::includeModule('aspro.tires2'))
{
	...
	if(!$arResult['POPUP']  || ($arResult['POPUP'] && \Bitrix\Main\Config\Option::get('aspro.tires2', 'REGIONALITY_SEARCH_ROW', 'N') != 'Y'))
	{
		$arResult['REGIONS'] = CTires2Regionality::getRegions();
		$arResult['CURRENT_REGION'] = CTires2Regionality::getCurrentRegion();
		$arResult['REAL_REGION'] = CTires2Regionality::getRealRegionByIP();
	...
	if($arResult['REGIONS'])
	{
		...
		if($arResult['POPUP'])
		{
			...
			if($arSectionsID)
			{
				$arSections = CTires2Cache::CIBlockSection_GetList(array('SORT' => 'ASC', 'NAME' => 'ASC', 'CACHE' => array('GROUP' => 'ID', 'TAG' => CTires2Cache::GetIBlockCacheTag(CTires2Cache::$arIBlocks[SITE_ID]['aspro_tires2_regionality']['aspro_tires2_regions'][0]))), array('GLOBAL_ACTIVE' => 'Y', 'ID' => $arSectionsID, 'IBLOCK_ID' => CTires2Cache::$arIBlocks[SITE_ID]['aspro_tires2_regionality']['aspro_tires2_regions'][0]), false, array('ID', 'IBLOCK_ID', 'NAME', 'LEFT_MARGIN', 'RIGHT_MARGIN', 'DEPTH_LEVEL'));
				if($arSections)
				{
					$arSections2 = array();
					foreach($arSections as $key => $arSection)
					{
						...
						if($arSection['DEPTH_LEVEL'] > 1)
						{
							...
							$arTmpSection = CTires2Cache::CIBlockSection_GetList(array('CACHE' => array('MULTI' => 'N', 'TAG' => CTires2Cache::GetIBlockCacheTag(CTires2Cache::$arIBlocks[SITE_ID]['aspro_tires2_regionality']['aspro_tires2_regions'][0]))), array('GLOBAL_ACTIVE' => 'Y', '<=LEFT_BORDER' => $arSection['LEFT_MARGIN'], '>=RIGHT_BORDER' => $arSection['RIGHT_MARGIN'], 'DEPTH_LEVEL' => 1, 'IBLOCK_ID' => CTires2Cache::$arIBlocks[SITE_ID]['aspro_tires2_regionality']['aspro_tires2_regions'][0]), false, array('ID', 'IBLOCK_ID', 'NAME', 'SORT'));
	...
	elseif($arResult['POPUP'] && \Bitrix\Main\Config\Option::get('aspro.tires2', 'REGIONALITY_SEARCH_ROW', 'N') == 'Y')
	{
		$APPLICATION->AddHeadScript(SITE_TEMPLATE_PATH.'/js/jquery-ui.min.js');
		$this->IncludeComponentTemplate();
	}
