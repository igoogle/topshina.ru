		...
		<?foreach($arResult["ITEMS"] as $arItem):?>
			<div class="item" itemprop="review" itemscope="itemscope" itemtype="http://schema.org/Review">
					...
					<div class="n-product-review-user_profile">
						...
						<?else:?>
							...
							<?if($arItem["authorInfo"]["grades"] > 1):?>
								<span class="number_reviews"><?=\Aspro\Functions\CAsproTires2::declOfNum($arItem["authorInfo"]["grades"], array(Loc::getMessage("ONEW_REVIEW"), Loc::getMessage("TWO_REVIEW"), Loc::getMessage("THREE_REVIEW")));?></span>
							<?endif;?>
						<?endif;?>
					</div>
				</div>
				<div class="middle_block_wrapper">
					...
					<span class="n-product-review-item__delivery"><?=Loc::getMessage("USAGE_TIME", array("#TIME#" => \Aspro\Functions\CAsproTires2::formatUsageTime($arItem["usageTime"])))?></span>
				</div>
				<?if($arItem["pro"]):?>
					<div class="middle_block_wrapper">
