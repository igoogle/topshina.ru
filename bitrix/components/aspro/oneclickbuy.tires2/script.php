...
else
{
	...
	$bUseCaptcha = (\Bitrix\Main\Config\Option::get('aspro.tires2', 'ONE_CLICK_BUY_CAPTCHA', 'N') == 'Y');
	...
	if($bAllBasketBuy){
		...
		if(strlen($request['OFFER_PROPERTIES']) && $iblockID > 0){
			$arOfferProperties=json_decode($request["OFFER_PROPERTIES"]);
		...
		if(is_array($arOrderDat) && array_key_exists("ORDER_PRICE", $arOrderDat)){
			...
			$arError = CTires2::checkAllowDelivery($arOrderDat["ORDER_PRICE"], $arOrderDat["CURRENCY"]);
