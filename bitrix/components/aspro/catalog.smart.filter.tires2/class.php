...
class CBitrixCatalogSmartFilter extends CBitrixComponent
{
	...
	*/
	...
	function makeFilter($FILTER_NAME)
	{
		...
		if ($gFilter && CTires2::checkVersionModule('18.6.200', 'iblock')) {
			foreach ($gFilter as $key => $value) {
				if (strpos($key, 'CATALOG_PRICE_') !== false && $this->arParams["CONVERT_CURRENCY"] == "Y" && $this->arParams["CURRENCY_ID"]) {
					unset($gFilter[$key]);
					$key = str_replace('CATALOG_PRICE_', 'PRICE_', $key);
					$priceID = end(explode('_', $key));
					$gFilter[$key] = $value;
					if (!$gFilter['CURRENCY_FOR_SCALE_'.$priceID]) {
						$gFilter['CURRENCY_FOR_SCALE_'.$priceID] = $this->arParams["CURRENCY_ID"];
					}
				}
			}
		}
		...
		if(self::$catalogIncluded && $bOffersIBlockExist)
		{
			...
			foreach($gFilter as $key => $value)
			{
				if (class_exists('CProductQueryBuilder') && method_exists('CProductQueryBuilder', 'isPriceFilterField')) {
					if (\CProductQueryBuilder::isPriceFilterField($key)) {
						$arPriceFilter[$key] = $value;
						unset($gFilter[$key]);
					}
				} elseif(preg_match('/^(>=|<=|><)CATALOG_PRICE_/', $key)) {
					$arPriceFilter[$key] = $value;
					unset($gFilter[$key]);
				}
			}
