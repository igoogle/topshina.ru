...
if($this->StartResultCache(false, 'v2'.($arParams["CACHE_GROUPS"]? $USER->GetGroups(): false)))
{
	...
	{
		if ($this->facetAspro->isValid() && $arParams["NO_FACET"] != "Y")
		{
	...
	if(isset($arItem["PRICE"]))
	{
		if (CTires2::checkVersionModule('18.6.200', 'iblock')) {
			if(strlen($arItem["VALUES"]["MIN"]["HTML_VALUE"]) && strlen($arItem["VALUES"]["MAX"]["HTML_VALUE"]))
				${$FILTER_NAME}["><PRICE_".$arItem["ID"]] = array($arItem["VALUES"]["MIN"]["HTML_VALUE"], $arItem["VALUES"]["MAX"]["HTML_VALUE"]);
			elseif(strlen($arItem["VALUES"]["MIN"]["HTML_VALUE"]))
				${$FILTER_NAME}[">=PRICE_".$arItem["ID"]] = $arItem["VALUES"]["MIN"]["HTML_VALUE"];
			elseif(strlen($arItem["VALUES"]["MAX"]["HTML_VALUE"]))
				${$FILTER_NAME}["<=PRICE_".$arItem["ID"]] = $arItem["VALUES"]["MAX"]["HTML_VALUE"];
			if ($arParams["CONVERT_CURRENCY"] == "Y" && $arParams["CURRENCY_ID"]) {
				${$FILTER_NAME}['CURRENCY_FOR_SCALE_'.$arItem["ID"]] = $arParams["CURRENCY_ID"];
			}
		} else {
			if(strlen($arItem["VALUES"]["MIN"]["HTML_VALUE"]) && strlen($arItem["VALUES"]["MAX"]["HTML_VALUE"]))
