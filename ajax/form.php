...
if(\Bitrix\Main\Loader::includeModule("aspro.tires2"))
{
	...
	CTires2::GetValidFormIDForSite($form_id);
}
$url_sizes = (htmlspecialchars($request['url']) ? htmlspecialchars($request['url']) : '');
?>
...
<?elseif(isset($request['type']) && $request['type'] == 'auth'):?>
	<?include_once('auth.php');?>
<?elseif($form_id == 'filter_all_hint'):?>
	<a href="#" class="close jqmClose"><i></i></a>
	<div class="form">
		<div class="form_head">
			<h2><?=\Bitrix\Main\Localization\Loc::getMessage('FILTER_ALL_HINT_TITLE');?></h2>
		</div>
		<div class="form_body">
			<?include_once('../include/filter_all_hint/filter_all_hint.php');?>
		</div>
	</div>
